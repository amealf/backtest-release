...
# open_continous_threshold_runtime = open_threshold_runtime + i * step3
for_num_3 = 29
step3 = 0.25
...
open_continous_threshold2 = 1.0
close_withdrawal_threshold2 = 1.0
...
GARCH_FORECAST_DIR = Path('./result') / 'garch forecast'
GARCH_PERIOD_MINUTES = list(range(3, 61))


def build_result_root(file_name: str) -> Path:
    return Path('./result') / f'{file_name} long_momentum_GARCH outcome'
...
def build_garch_forecast_path(period_minutes: int) -> Path:
    return GARCH_FORECAST_DIR / f'period_{int(period_minutes)}min garch forecast.parquet'


def load_garch_forecast_frame(period_minutes: int) -> pd.DataFrame:
    parquet_path = build_garch_forecast_path(period_minutes)
    if not parquet_path.exists():
        raise FileNotFoundError('GARCH forecast file not found: ' + str(parquet_path))

    frame = pd.read_parquet(
        parquet_path,
        columns=[
            'Date',
            'garch_sigma_price_trade_bar',
            'garch_sigma_return_trade_bar',
        ],
    ).copy()
    frame['merge_date'] = pd.to_datetime(frame['Date'], errors='coerce')
    frame = frame.dropna(subset=['merge_date']).drop_duplicates(
        subset=['merge_date'],
        keep='last',
    )
    frame = frame.rename(columns={
        'garch_sigma_price_trade_bar': f'garch_sigma_price_m{int(period_minutes)}',
        'garch_sigma_return_trade_bar': f'garch_sigma_return_m{int(period_minutes)}',
    })
    return frame[
        [
            'merge_date',
            f'garch_sigma_price_m{int(period_minutes)}',
            f'garch_sigma_return_m{int(period_minutes)}',
        ]
    ].reset_index(drop=True)


def merge_garch_forecast_columns(
        quote: pd.DataFrame,
        period_minutes_list: list[int]) -> pd.DataFrame:
    merged = quote.copy()
    merged['merge_date'] = pd.to_datetime(merged['Date'], errors='coerce')
    if merged['merge_date'].isna().any():
        raise ValueError('Date column cannot be parsed when merging GARCH forecast.')

    for period_minutes in period_minutes_list:
        forecast_df = load_garch_forecast_frame(period_minutes)
        merged = merged.merge(
            forecast_df,
            on='merge_date',
            how='left',
    ...
    return merged.drop(columns=['merge_date'])


def get_sigma_column_name(period_minutes: int, sigma_kind: str = 'price') -> str:
    if sigma_kind == 'return':
        return f'garch_sigma_return_m{int(period_minutes)}'
    return f'garch_sigma_price_m{int(period_minutes)}'
...
def build_long_param_tag(
        ...
        close_withdrawal_threshold: float) -> str:
    return (
...
def export_interactive_html_long(
    ...
    html_dir = build_result_root(file_name) / 'html'
    os.makedirs(html_dir, exist_ok=True)
    html_path = str(html_dir / (save_name + ' Long interactive.html'))
...
class MomentumStrategy(BaseStrategy):
    ...
    def _get_sigma_abs(
            ...
            quote: pd.DataFrame,
            integer_index: int,
            period_minutes: int) -> float:
        column_name = get_sigma_column_name(period_minutes, 'price')
        if column_name not in quote.columns:
            return np.nan
        value = quote.iat[integer_index, quote.columns.get_loc(column_name)]
        if pd.isna(value):
            return np.nan
        value = float(value)
        if value <= 0:
    ...
    def _get_threshold_bundle(
            self,
            quote: pd.DataFrame,
            integer_index: int,
            params: dict) -> dict:
        sigma_open = self._get_sigma_abs(
            quote,
            integer_index,
            int(params['open_bar']),
        )
        sigma_close = self._get_sigma_abs(
            quote,
            integer_index,
            int(params['close_bar']),
        )
        return {
            'sigma_open': sigma_open,
            'sigma_close': sigma_close,
            'open_threshold_abs': (
                sigma_open * float(params['open_threshold'])
                if pd.notna(sigma_open) else np.nan
            ),
            'open_cont_threshold_abs': (
                sigma_open * float(params['open_continous_threshold'])
                if pd.notna(sigma_open) else np.nan
            ),
            'open_withdrawal_threshold_abs': (
                sigma_open * float(params['open_withdrawal_threshold'])
                if pd.notna(sigma_open) else np.nan
            ),
            'close_threshold_abs': (
                sigma_close * float(params['close_threshold'])
                if pd.notna(sigma_close) else np.nan
            ),
            'close_withdrawal_threshold_abs': (
                sigma_close * float(params['close_withdrawal_threshold'])
                if pd.notna(sigma_close) else np.nan
            ),
        }
    ...
    def get_extra_columns(self) -> list:
        return [
            ...
            'sigma_open_abs', 'sigma_close_abs',
            'open_th_abs', 'open_cont_th_abs',
            'open_wd_th_abs', 'close_th_abs',
            'var0', 'period',
    ...
    def on_bar_idle(self, ctx: BarContext) -> OpenResult | None:
        ...
        thresholds = self._get_threshold_bundle(quote, ii, p)
        signal.at[index, 'sigma_open_abs'] = thresholds['sigma_open']
        signal.at[index, 'sigma_close_abs'] = thresholds['sigma_close']
        signal.at[index, 'open_th_abs'] = thresholds['open_threshold_abs']
        signal.at[index, 'open_cont_th_abs'] = thresholds['open_cont_threshold_abs']
        signal.at[index, 'open_wd_th_abs'] = thresholds['open_withdrawal_threshold_abs']
        signal.at[index, 'close_th_abs'] = thresholds['close_threshold_abs']

        if (
            pd.isna(thresholds['open_threshold_abs'])
            or pd.isna(thresholds['open_cont_threshold_abs'])
            or pd.isna(thresholds['open_withdrawal_threshold_abs'])
            or pd.isna(thresholds['close_threshold_abs'])
        ):
            self.var0 = 0
        ...
        if self.var0 == 0:
            ...
            signal.at[index, 'wd_per'] = round(
                withdrawal / thresholds['open_withdrawal_threshold_abs'],
                4,
            )
            signal.at[index, 'increase'] = increase
            signal.at[index, 'inc_per'] = round(
                increase / thresholds['open_threshold_abs'],
                4,
            )

            cond1 = (increase >= thresholds['open_threshold_abs'])
            signal.at[index, 'inc_signal'] = 1 if cond1 else 0

            cond2 = withdrawal < thresholds['open_withdrawal_threshold_abs']
            ...
            if signal.at[index, 'wd_signal']:
                ...
                if increase > thresholds['open_cont_threshold_abs']:
        ...
        if self.var0 == 2:
            ...
            with_high, withdrawal = get_withdrawal(
                cond3_analysis_slice, 0, ii)
            signal.at[index, 'withdrawal'] = withdrawal
            total_increase, inc_base = get_increase_with_base(cond3_analysis_slice)

            cond3 = withdrawal < thresholds['open_withdrawal_threshold_abs']
            ...
            if signal.at[index, 'wd_signal']:
                if self.new_opening_count >= open_bar:
                    ...
                    signal.at[index, 'a_inc_per'] = round(
                        analysis_increase / thresholds['close_threshold_abs'],
                        4,
                    )
                    if analysis_increase < thresholds['close_threshold_abs']:
                        self.var0 = 4

                signal.at[index, 'total_inc'] = total_increase
                signal.at[index, 't_inc_per'] = round(
                    total_increase / thresholds['open_cont_threshold_abs'],
                    4,
                )
                self.first_cond1_price = inc_base

                if total_increase >= thresholds['open_cont_threshold_abs']:
            ...
            if signal.at[index, 'total_inc_signal'] == 1:
                return OpenResult(
                    execution_price=round(
                        self.first_cond1_price
                        + thresholds['open_cont_threshold_abs'],
    ...
    def on_bar_holding(self, ctx: BarContext) -> CloseResult | None:
        ...
        signal.at[index, 'sigma_close_abs'] = thresholds['sigma_close']
        signal.at[index, 'close_th_abs'] = thresholds['close_threshold_abs']
        signal.at[index, 'close_wd_th_abs'] = thresholds['close_withdrawal_threshold_abs']

        if (
            pd.isna(thresholds['close_threshold_abs'])
            or pd.isna(thresholds['close_withdrawal_threshold_abs'])
        ):
            return None
        ...
        if window_ready:
            ...
            self.holding_increase_percent = (
                holding_increase / thresholds['close_threshold_abs']
            )
            signal.at[index, 'holding_inc'] = holding_increase
            if holding_increase < thresholds['close_threshold_abs']:
                signal.at[index, 'speed_close_signal'] = 1

        # 回撤条件
        holding_high = float(holding_slice['high'].max())
        close_withdrawal_threshold_abs = thresholds['close_withdrawal_threshold_abs']
        signal.at[index, 'close_wd_floor_per'] = round(
            close_withdrawal_threshold_floor,
            4,
        )
        ...
        with_high, holding_withdrawal = get_withdrawal(
            holding_slice, 0, ii, switch0=True)
        holding_withdrawal_percent = (
            holding_withdrawal / close_withdrawal_threshold_abs
            if close_withdrawal_threshold_abs != 0 else np.nan
        )
        ...
        if holding_withdrawal > close_withdrawal_threshold_abs:
        ...
        if signal.at[index, 'holding_wd_signal'] == 1:
            exec_price = holding_high - close_withdrawal_threshold_abs
            if exec_price > quote.loc[index, 'open']:
                exec_price = quote.loc[index, 'open']
            return CloseResult(
                close_type=1,
...
if __name__ == '__main__':
    ...
    direct_loaded = try_load_direct_period_source(
        folder_path,
        file_name,
        '1min',
    )
    if direct_loaded is None:
        raise FileNotFoundError(
            '1min direct source file not found: '
            + str(Path(folder_path) / f'{file_name}_1_min.csv')
        )
    native_df, ROUND_PRECISION, NATIVE_BAR_SECONDS, source_path = direct_loaded
    native_df = native_df.copy()
    native_df['Date'] = native_df['Date'].dt.strftime('%Y-%m-%d %H:%M:%S')
    print('[Data] using direct source: ' + str(source_path))
    ...
    if normalize_rule_token(resample_rule) != '1min':
        raise ValueError('long_momentum_GARCH.py currently requires resample_rule = 1min.')
    ...
    os.makedirs(result_root / 'image', exist_ok=True)
    ...
    underlying = merge_garch_forecast_columns(
        df5.copy(),
        GARCH_PERIOD_MINUTES,
    )
    run_name = (
    ...
    dashboard_outcome_stats_path = str(
        result_root / 'outcome stats' / ('long_momentum_GARCH ' + run_name + ' outcome_stats.xlsx')
    )
    summary_dir = str(Path('./result') / f'stats {file_name} long_momentum_GARCH outcome')
    ...
    html_underlying = None
    html_factor = None
    if export_interactive_html_enabled:
        html_underlying = underlying.reset_index(drop=True).copy()
        html_factor = html_underlying['open'].iloc[0]
    ...
    if run_mode == 'manual':
        ...
        if step2 == 0:
            raise ValueError('step2 cannot be 0.')
        if step3 <= 0:
            raise ValueError('step3 must be positive.')
        if int(for_num_1) <= 0:
            raise ValueError('for_num_1 must be positive.')
        if int(for_num_2) <= 0:
        ...
        open_bar_values = [
            int(round(open_bar_cfg + (ii * step1)))
            for ii in range(int(for_num_1))
        ]
        if len(open_bar_values) == 0 or min(open_bar_values) <= 0:
        ...
        if (
            ...
            outcome_stats = merge_outcome_stats(
                outcome_stats,
                load_progress_summary(progress_summary_path),
            )
    ...
    open_bar_end_runtime = open_bar_values[-1]
    print(
        '[Main] open_bar start=' + str(open_bar_cfg)
        + ' end=' + str(open_bar_end_runtime)
        + ' step=' + str(step1)
        + ' count=' + str(len(open_bar_values))
    )
    if run_mode == 'grid':
        print(
            '[Grid] open_threshold start=' + str(open_threshold)
            + ' step=' + str(step2)
            + ' count=' + str(for_num_2_runtime)
        )
        print(
            '[Grid] open_continous_threshold start=' + str(open_continous_threshold)
            + ' step=' + str(step3)
    ...
    if run_mode == 'manual':
        ...
        planned_param_tags = build_planned_param_tags_long(
            open_bar_values,
        ...
        for threshold_iter in threshold_iterations:
            if run_mode == 'grid':
                open_threshold_runtime = round(
                    open_threshold + (threshold_iter * step2),
                    10,
                )
                print(
            ...
            for open_cont_iter in open_cont_iterations:
                if run_mode == 'grid':
                    open_continous_threshold_runtime = round(
                        open_continous_threshold + (open_cont_iter * step3),
                        10,
                    )
                    print(
                ...
                if run_mode == 'grid':
                    ...
                    close_withdrawal_threshold_value = open_threshold_runtime
                else:
                ...
                if save_static_plot:
                    ...
                    fig1_path = str(
                        result_root
                        / (' ' + str(Capital_outcome) + save_name + f' Long.{plot_ext}')
                    )
                ...
                writer1 = pd.ExcelWriter(
                    str(result_root / 'perf' / perf_name),
                    engine='xlsxwriter')
                ...
                if run_mode == 'grid':
                    ...
                    if unchanged_open_cont_steps >= open_threshold_stop_flat_rounds:
                        for remain_open_cont_iter in range(
                                open_cont_iter + 1,
                                int(for_num_3_runtime)):
                            remain_open_cont_tag = build_long_param_tag(
                                open_bar_runtime,
                                open_threshold_runtime,
                                round(
                                    open_continous_threshold
                            ...
                            completed_param_tags.add(remain_open_cont_tag)
                        print_search_progress(
                            len(completed_param_tags),
                            total_search_space,
                            progress_marks,
            ...
            if run_mode == 'grid':
                ...
                if unchanged_open_threshold_steps >= open_threshold_stop_flat_rounds:
                    for remain_threshold_iter in range(
                            threshold_iter + 1,
                            int(for_num_2_runtime)):
                        remain_open_threshold = round(
                            open_threshold + (remain_threshold_iter * step2),
                            10,
                        )
                        for remain_open_cont_iter in range(int(for_num_3_runtime)):
                            remain_open_cont = round(
                                open_continous_threshold
                                + (remain_open_cont_iter * step3),
                                10,
                            )
                            if remain_open_cont < remain_open_threshold:
                                continue
                            completed_param_tags.add(build_long_param_tag(
                                open_bar_runtime,
                                remain_open_threshold,
                                remain_open_cont,
                                remain_open_threshold,
                                close_bar_runtime,
                                remain_open_threshold,
            ...
            if run_mode == 'grid':
                print(
                    '[Grid] reached for_num_2='
                    + str(for_num_2_runtime)
                    + f' at open_bar={open_bar_runtime}.'
                )
    ...
    if run_mode == 'manual' and executed_run_count == 1:
        ...
        if len(sell_record) != 0:
            def update_annot_g(ind):
                ...
                close_wd_floor = round(pref_data['close_wd_floor_per'], 2)
                close_wd_dyn = round(pref_data['close_wd_dyn_per'], 2)
                close_wd_th = round(pref_data['close_wd_th_per'], 2)
                close_wd_th_abs = round(pref_data['close_wd_th_abs'], 4)
                execution = pref_data['execution']
                low_date = pref_data['low_date']
                ...
                text = (date[:-5] + ' ' + date[-5:] + '\n'
                        ...
                        + 'max_inc: ' + str(max_inc) + '%' + '\n'
                        + 'max_wd: ' + str(max_wd) + '%' + '\n'
                        + 'close_wd_floor: ' + str(close_wd_floor) + '%' + '\n'
                        + 'close_wd_dyn: ' + str(close_wd_dyn) + '%' + '\n'
                        + 'close_wd_th: ' + str(close_wd_th) + '%' + '\n'
                        + 'close_wd_th_abs: ' + str(close_wd_th_abs) + '\n'
