...
open_vol_multiplier = 2.5
open_continous_vol_multiplier = open_vol_multiplier
# 连续多少次上调波动率倍数后，总交易数保持不变，就停止当前 bar 的搜索。
open_vol_stop_flat_rounds = 5
# 安全上限，防止极端数据导致波动率搜索循环过长。
open_vol_max_iterations = 100
open_wd_vol_multiplier = open_vol_multiplier
close_speed_vol_multiplier = open_vol_multiplier

# Grid search
bar_step = 3
bar_end = 99
open_vol_step = 0.1
open_cont_vol_max_iterations = open_vol_max_iterations
open_cont_vol_step = 0.1
...
VOLATILITY_METHOD_SPECS = {
    'garch': {
        ...
        'price_col': 'garch_sigma_price',
        'return_col': 'garch_sigma_return',
    }
}
...
def merge_volatility_forecast(
    ...
    forecast_df['merge_date'] = pd.to_datetime(
        forecast_df['Date'], errors='coerce'
    )
    forecast_df = forecast_df.rename(columns={
        spec['price_col']: 'vol_forecast_price',
        spec['return_col']: 'vol_forecast_return',
    })
    forecast_df = forecast_df[
        ['merge_date', 'vol_forecast_price', 'vol_forecast_return']
...
def build_long_param_tag(
    ...
    return (
        ...
        + ' ov' + str(round(open_threshold, 4))
        + ' ocv' + str(round(open_continous_threshold, 4))
        + ' owv' + str(round(open_withdrawal_threshold, 4))
        + ' cv' + str(round(close_threshold, 4))
...
def build_planned_param_tags_long_vol(
        open_bar_values: list[int],
        open_vol_multiplier_start: float,
        open_vol_iterations: int,
        open_vol_step_value: float,
        open_cont_vol_iterations: int,
        open_cont_vol_step_value: float,
    ...
    for open_bar_value in open_bar_values:
        ...
        for vol_iter in range(int(open_vol_iterations)):
            open_threshold_value = round(
                open_vol_multiplier_start + (vol_iter * open_vol_step_value),
                10,
            )
            ...
            if min(
                ...
                continue
            for open_cont_iter in range(int(open_cont_vol_iterations)):
                open_continous_threshold_value = round(
                    open_threshold_value
                    + (open_cont_iter * open_cont_vol_step_value),
                    10,
                )
    ...
    if len(long_record) != 0:
        ...
        for idx, row in long_record.iterrows():
            ...
            long_texts.append(
                ...
                + 'vol: ' + _safe_val(pref_data, 'vol_forecast_price', 6) + '<br>'
                + 'frozen_vol: ' + _safe_val(pref_data, 'frozen_open_vol', 6) + '<br>'
                + 'ov_mult: ' + _safe_val(pref_data, 'open_vol_multiplier_runtime', 4) + '<br>'
                + 'ocv_mult: ' + _safe_val(pref_data, 'open_cont_vol_multiplier_runtime', 4) + '<br>'
                + 'owv_mult: ' + _safe_val(pref_data, 'open_wd_vol_multiplier_runtime', 4) + '<br>'
        ...
        if len(close_type_1_df) != 0:
            ...
            for idx, row in close_type_1_df.iterrows():
                ...
                sell_1_texts.append(
                    _date_text(row['Date']) + '<br>'
                    + 'vol: ' + _safe_val(pref_data, 'vol_forecast_price', 6) + '<br>'
                    + 'cv_mult: ' + _safe_val(pref_data, 'close_speed_vol_multiplier_runtime', 4) + '<br>'
...
class MomentumStrategy(BaseStrategy):
    ...
    def _build_frozen_open_thresholds(
        ...
        if low_index <= 0 or 'vol_forecast_price' not in quote.columns:
            return None

        frozen_open_vol = quote.iloc[low_index - 1]['vol_forecast_price']
        if pd.isna(frozen_open_vol):
            return None

        return {
            'frozen_open_vol': float(frozen_open_vol),
            'open_threshold': float(frozen_open_vol) * self.params['open_threshold'],
            'open_continous_threshold': (
                float(frozen_open_vol) * self.params['open_continous_threshold']
            ),
            'open_withdrawal_threshold': (
                float(frozen_open_vol) * self.params['open_withdrawal_threshold']
            ),
        }

    def _apply_frozen_open_thresholds(self, frozen_thresholds: dict):
        self.frozen_open_vol = frozen_thresholds['frozen_open_vol']
    ...
    def _write_open_threshold_state(
        ...
        signal.at[index, 'frozen_open_vol'] = frozen_open_vol
    ...
    def _resolve_thresholds(self, ctx: BarContext) -> dict:
        ...
        vol_basis = (
            quote.at[index, 'vol_forecast_price']
            if 'vol_forecast_price' in quote.columns else np.nan
        )
        pos_basis = vol_basis
        neg_basis = vol_basis
        basis_ready = int(not pd.isna(vol_basis))

        open_threshold = vol_basis * p['open_threshold']
        open_continous_threshold = vol_basis * p['open_continous_threshold']
        open_withdrawal_threshold = vol_basis * p['open_withdrawal_threshold']
        close_threshold = vol_basis * p['close_threshold']
    ...
    def on_bar_idle(self, ctx: BarContext) -> OpenResult | None:
        ...
        if self._has_frozen_open_thresholds():
            ...
            row_frozen_open_vol = self.frozen_open_vol
        ...
        if self.var0 == 0:
            ...
            if candidate_frozen_thresholds is not None:
                ...
                row_frozen_open_vol = candidate_frozen_thresholds['frozen_open_vol']
