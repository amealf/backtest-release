...
class MomentumStrategy(BaseStrategy):
    ...
    def on_bar_idle(self, ctx: BarContext) -> OpenResult | None:
        ...
        close_withdrawal_threshold_floor = p['close_withdrawal_threshold']
...
if __name__ == '__main__':
    ...
    progress_token = make_safe_range_token(
        ...
        + '_f2' + str(for_num_2)
        + '_oc' + str(open_continous_threshold)
        + '_s2' + str(step2)
        + '_f3' + str(for_num_3)
        + '_s3' + str(step3)
    ...
    progress_signature = {
        ...
        'open_continous_threshold_start': float(open_continous_threshold),
    ...
    if run_mode == 'manual':
        ...
        open_bar_values = parse_env_open_bar_values(open_bar_values)
    ...
    if grid_shard_tag:
        print('[Grid] shard tag=' + grid_shard_tag)
    if grid_open_bar_values_env:
        print('[Grid] shard open_bar values=' + str(open_bar_values))
    if run_mode == 'grid':
        print(
        ...
        print(
            '[Grid] open_continous_threshold start='
            + str(open_continous_threshold)
            + ' step=' + str(step3)
                ...
                if unchanged_open_threshold_steps >= open_threshold_stop_flat_rounds:
                    for remain_threshold_iter in range(
                        ...
                        for remain_open_cont_iter in range(int(for_num_3_runtime)):
                            ...
                            if remain_open_cont < close_withdrawal_threshold:
                                continue
