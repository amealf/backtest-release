# -*- coding: utf-8 -*-
"""
Short Momentum Strategy - 动量做空策略
=====================================
策略入口脚本：包含 ReverseMomentumStrategy 类、参数循环、绘图、Excel 输出。
...
def get_analysis_decrease(df):
    ...
    analysis_decrease = high - low
    return analysis_decrease
def get_rebound(df, close_withdrawal_threshold0,
    ...
    for index, row in df.iterrows():
        if not initialized:
            with_high = row['close']
            with_low = row['close']
            rebound = with_high - with_low
            initialized = True
        else:
            if row['low'] < with_low:
                with_low = row['low']
                with_high = row['close']
            elif row['high'] > with_high:
                with_high = row['high']
            rebound = with_high - with_low
    return with_low, rebound
def get_max_rebound(df, assumebarwithdrawal=True):
    if df.empty:
        print('received empty dataframe at get_decrease function.')
        return np.nan
    need_cols = ['open', 'high', 'low', 'close']
    ...
    for index, row in df.iterrows():
        if not initialized:
            ...
            if row['low'] < with_low:
                ...
                with_high = row['high']
            rebound = (with_high - with_low) / with_low if with_low != 0 else 0
        if rebound > max_rb:
            max_rb = rebound
    return max_rb
def get_outcome_rebound(sers):
    initialized = False
    with_high = 0
    with_low = 0
    rebound = 0
...
def export_interactive_html_short(
        file_name: str,
        save_name: str,
        title: str,
    ...
    short_record = transactions_df.copy()
    short_record['target'] = short_record['Price'] / factor * 100
    short_record = short_record[short_record.Type == 'short']
    if len(short_record) != 0:
        short_texts = []
        for idx, row in short_record.iterrows():
            ...
            short_texts.append(
                ...
                + 'total_dec: ' + _safe_val(pref_data, 't_dec_per', 2) + '%' + '<br>'
                + 'execution: ' + _safe_val(pref_data, 'execution') + '<br>'
                + 'high_date: ' + _safe_val(pref_data, 'low_date') + '<br>'
                + 'high_price: ' + _safe_val(pref_data, 'low_price') + '<br>'
                + 'new_opening_count: ' + _safe_val(pref_data, 'new_opening_count') + '<br>'
                + 'index: ' + str(idx)
            )
        fig_html.add_trace(go.Scatter(
            x=short_record.index,
            y=short_record['target'],
            mode='markers',
    ...
    buy_record = transactions_df.copy()
    buy_record['target'] = buy_record['Price'] / factor * 100
    buy_record = buy_record[buy_record.Type == 'buy']
    buy_1_count = 0
    buy_2_count = 0
    if len(buy_record) != 0:
        close_type_1_df = buy_record[buy_record['Close_type'] == 1]
        buy_1_count = int(len(close_type_1_df))
        if len(close_type_1_df) != 0:
            ...
            for idx, row in close_type_1_df.iterrows():
                ...
                buy_1_texts.append(
                    _date_text(row['Date']) + '<br>'
                    + 'low: ' + _safe_val(pref_data, 'low') + '<br>'
                    + 'hld_rb_per: ' + _safe_val(pref_data, 'hld_rb_per', 2) + '%' + '<br>'
                    + 'holding_dec: ' + _safe_val(pref_data, 'holding_dec', 2) + '<br>'
                    + 'max_dec: ' + _safe_val(pref_data, 'max_dec', 2) + '%' + '<br>'
                    + 'max_rb: ' + _safe_val(pref_data, 'max_rb', 2) + '%' + '<br>'
                    + 'execution2: ' + _safe_val(pref_data, 'execution') + '<br>'
                    + 'period: ' + _safe_val(pref_data, 'period') + '<br>'
                    + 'high_date: ' + _safe_val(pref_data, 'low_date') + '<br>'
                    + 'low_date: ' + _safe_val(pref_data, 'high_date') + '<br>'
                    + 'low_price: ' + _safe_val(pref_data, 'high_price') + '<br>'
                    + 'index: ' + str(idx)
                )
            fig_html.add_trace(go.Scatter(
        ...
        close_type_2_df = buy_record[buy_record['Close_type'] == 2]
        buy_2_count = int(len(close_type_2_df))
    ...
    trade_seq = transactions_df[
        transactions_df['Type'].isin(['short', 'buy'])].copy()
    ...
    for idx, row in trade_seq.iterrows():
        if row['Type'] == 'short':
            short_idx = idx
            short_y = row['target']
        elif row['Type'] == 'buy' and short_idx is not None:
            line_x.extend([short_idx, idx, None])
            line_y.extend([short_y, row['target'], None])
            short_idx = None
            short_y = None
    if len(line_x) > 0:
        fig_html.add_trace(go.Scatter(
    ...
    if HTML_SHOW_TRADE_COUNT_BADGE:
        total_trade_count = buy_1_count + buy_2_count
    ...
    html_dir = './result/%s short outcome/html' % file_name
    os.makedirs(html_dir, exist_ok=True)
    html_path = os.path.join(html_dir, save_name + ' Short interactive.html')
...
class ReverseMomentumStrategy(BaseStrategy):
    ...
    def get_extra_columns(self) -> list:
        return [
            ...
            'decrease', 'dec_per', 'dec_signal',
            'ana_dec', 'a_dec_per',
            'total_dec', 't_dec_per', 'total_dec_signal',
            'max_dec', 'max_rb',
            'holding_rb', 'hld_rb_per', 'holding_rb_signal',
            'holding_dec', 'speed_close_signal',
            'var0', 'period',
    ...
    def on_bar_idle(self, ctx: BarContext) -> OpenResult | None:
        ...
        if self.var0 == 0:
            ...
            rb_percent = rebound / with_low if with_low != 0 else 0
            signal.at[index, 'rebound'] = rebound
            signal.at[index, 'rb_per'] = round(rb_percent * 100, 4)
            signal.at[index, 'decrease'] = decrease
            signal.at[index, 'dec_per'] = round(dec_percent * 100, 4)
            cond1 = (dec_percent >= open_threshold)
            signal.at[index, 'dec_signal'] = 1 if cond1 else 0
            cond2 = rb_percent < open_withdrawal_threshold
            signal.at[index, 'rb_signal'] = 1 if cond2 else 0
            if signal.at[index, 'rb_signal']:
                if signal.at[index, 'dec_signal']:
                    for i in reversed(range(self.last_index, ii + 1)):
                        low_index_slice = quote.iloc[i:ii + 1]
                        decrease2 = get_decrease(low_index_slice)
                        if np.isclose(decrease2, decrease,
                ...
                if dec_percent > open_continous_threshold:
        ...
        if self.var0 == 2:
            ...
            if signal.at[index, 'rb_signal']:
                if self.new_opening_count >= open_bar:
                    ana_dec_slice_1 = quote.iloc[self.low_index:ii + 1]
                    ana_dec_slice_2 = quote.iloc[
            ...
            if self.var0 == 4:
                ...
                decrease1_slice = quote.iloc[self.last_index:ii + 1]
                decrease1 = get_decrease(decrease1_slice)
                for i in range(self.last_index, ii + 1):
                    low_index_slice = quote.iloc[i:ii + 1]
                    decrease2 = get_decrease(low_index_slice)
                    if np.isclose(decrease2, decrease1,
                                  rtol=0.0, atol=self.HIGH_MATCH_EPS):
                ...
                self.analysis_decrease = 0
            # 开仓信号
            if signal.at[index, 'total_dec_signal'] == 1:
                return OpenResult(
    ...
    def adjust_and_validate_open_execution(self, ctx: BarContext,
        ...
        if exec_price < quote.loc[index, 'low']:
            print('short open execution price < low, plz check.')
    ...
    def on_bar_holding(self, ctx: BarContext) -> CloseResult | None:
        ...
        if self.new_opening:
            self.last_index = self.low_index
            self.decrease_start_index = self.low_index
        ...
        holding_slice = quote.iloc[self.decrease_start_index:ii + 1]
        ...
        if window_ready:
            ...
            holding_decrease = (
                ana_dec_slice_2.low.min() - ana_dec_slice_1.low.min())
            holding_base = analysis_slice['high'].iloc[0]
            self.holding_decrease_percent = holding_decrease / holding_base if holding_base != 0 else 0
            signal.at[index, 'holding_dec'] = holding_decrease
            if self.holding_decrease_percent < close_threshold:
                signal.at[index, 'speed_close_signal'] = 1

        # 回撤条件
        with_low, holding_rebound = get_rebound(
            holding_slice, close_withdrawal_threshold, ii, switch0=True)
        holding_rebound_percent = (
            holding_rebound / with_low if with_low != 0 else 0)
        signal.at[index, 'holding_rb'] = holding_rebound
        signal.at[index, 'hld_rb_per'] = round(
            holding_rebound_percent * 100, 4)

        if (open_continous_threshold2 == 0
            or (window_ready
                and (self.holding_decrease_percent
                     < open_continous_threshold2))):
            if holding_rebound_percent > close_withdrawal_threshold:
                signal.at[index, 'holding_rb_signal'] = 1
        else:
            if holding_rebound_percent > close_withdrawal_threshold:
                signal.at[index, 'holding_rb_signal'] = 1

        period = ii - self.holding_start_index + 1
        signal.at[index, 'high_price'] = min(holding_slice['low'])

        # 回撤平仓
        if signal.at[index, 'holding_rb_signal'] == 1:
            exec_price = (min(holding_slice['low'])
    ...
    def on_trade_stats(self, ctx: BarContext,
        ...
        """平仓后的交易统计: high_index, max_rb, max_dec"""
        quote = ctx.quote
        signal = ctx.signal
        index = ctx.index
        ii = ctx.integer_index

        decrease3_slice = quote.iloc[start_index:ii + 1]
        decrease3 = get_analysis_decrease(decrease3_slice)
        high_index = start_index
        for i in range(start_index + 1, ii + 2):
            high_index_slice = quote.iloc[start_index:i]
            decrease4 = get_analysis_decrease(high_index_slice)
            if np.isclose(decrease4, decrease3,
                          rtol=0.0, atol=self.HIGH_MATCH_EPS):
                high_index = i - 1
                break
        ...
        holding_slice = quote.iloc[
            self.decrease_start_index:ii + 1]
        signal.at[index, 'high_price'] = min(holding_slice['low'])
    ...
    def _do_idle_stats(self, quote, signal, index, ii, stat_type):
        ...
        decrease3_slice = quote.iloc[self.start_index:ii + 1]
        decrease3 = get_analysis_decrease(decrease3_slice)
...
if __name__ == '__main__':
    ...
    os.makedirs(f'./result/{file_name} short outcome/perf', exist_ok=True)
    os.makedirs(f'./result/{file_name} short outcome/trans', exist_ok=True)
    ...
    for num in range(for_num_1):
        for i in range(for_num_2):
            ...
            n = arr.shape[0]
            win = open_bar
            window_decrease = np.full(n, np.nan, dtype=float)
            for end in range(win - 1, n):
                ...
                for j in range(1, win):
                    ...
                    if low_j <= win_low:
                        ...
                        win_high = high_j
                window_decrease[end] = win_high - win_low
            underlying['Window_Decrease'] = window_decrease
            ...
            strategy = ReverseMomentumStrategy(params)
            ...
            if len(transactions_df) > 1:
                Capital_outcome = round(
                    transactions_df[
                        transactions_df.Type != 'short'].Capital.iloc[-1], 2)
            else:
                Capital_outcome = 100
            ...
            if SAVE_STATIC_PLOT:
                ...
                fig1_path = ('./result/%s short outcome/' % file_name
                             + ' ' + str(Capital_outcome)
            ...
            detail_df.drop(
                ['holding_signal', 'dec_signal', 'rb_signal',
                 'holding_rb_signal', 'total_dec_signal',
            ...
            if len(detail_df) == 0:
                detail_df.drop(
                    ['holding_rb', 'holding_dec', 'execution'],
                    axis=1, inplace=True)
            ...
            writer1 = pd.ExcelWriter(
                './result/%s short outcome/perf/' % file_name + perf_name,
                engine='xlsxwriter')
            ...
            if len(transactions_df) != 0:
                writer2 = pd.ExcelWriter(
                    './result/%s short outcome/trans/' % file_name
                    + 'om' + str(round(open_bar, 4))
                    + ' o' + str(round(open_threshold, 4))
                    + ' oc' + str(round(open_continous_threshold, 4))
                    + ' ow' + str(round(open_withdrawal_threshold, 4))
            ...
            outcome_high, outcome_rb = get_outcome_rebound(
                perf_outcome.capital)
            outcome_stats.at[outcome_index, 'outcome_high'] = outcome_high
            outcome_stats.at[outcome_index, 'biggest_rb'] = outcome_rb
    ...
    if for_num_2 > 1:
        ...
        ax_stat_2.plot(outcome_stats.biggest_rb, color='orange',
                       label='biggest rb')
        ...
        os.makedirs('./result/stats %s short outcome/' % file_name, exist_ok=True)
        if SAVE_STATIC_PLOT:
            stats_plot_ext = 'pdf' if SAVE_PLOT_AS_PDF else 'png'
            plt.savefig('./result/stats %s short outcome/' % file_name
                        + ' ' + save_name + ' '
                        + str(for_num_1) + ' '
                        + str(for_num_2) + ' '
                        + f'all outcome.{stats_plot_ext}', dpi=1000)
        outcome_stats.to_excel('./result/stats %s short outcome/' % file_name
                               + ' ' + save_name + ' '
                               + str(for_num_1) + ' '
                               + str(for_num_2) + ' '
                               + 'all outcome.xlsx')
        ...
        if open_excel:
            os.startfile(
                disk_path + '%s short outcome/perf/' % file_name + perf_name)
    ...
    if for_num_2 == 1:
        ...
        long_record = long_record[long_record.Type == 'short']
        ...
        sell_record = sell_record[sell_record.Type == 'buy']
        ...
        if len(long_record) != 0:
            ...
            def update_annot_r(ind):
                ...
                low = pref_data.low
                t_dec_per = round(pref_data['t_dec_per'], 2)
                execution = pref_data['execution']
                high_date = pref_data['low_date']
                new_opening_count = pref_data['new_opening_count']
                high_price = pref_data['low_price']
                text = (date[:-5] + ' ' + date[-5:] + '\n'
                        + 'low: ' + str(low) + '\n'
                        + 'total_dec: ' + str(t_dec_per) + '%' + '\n'
        ...
        if len(sell_record) != 0:
            def update_annot_g(ind):
                ...
                pref_data = detail_df.loc[index0]
                low = pref_data.low
                hld_rb_per = round(pref_data['hld_rb_per'], 2)
                holding_dec = round(pref_data['holding_dec'], 2)
                max_dec = round(pref_data['max_dec'], 2)
                max_rb = round(pref_data['max_rb'], 2)
                execution = pref_data['execution']
                ...
                text = (date[:-5] + ' ' + date[-5:] + '\n'
                        + 'low: ' + str(low) + '\n'
                        + 'hld_rb_per: ' + str(hld_rb_per) + '%' + '\n'
                        + 'holding_dec: ' + str(holding_dec) + '\n'
                        + 'max_dec: ' + str(max_dec) + '%' + '\n'
                        + 'max_rb: ' + str(max_rb) + '%' + '\n'
                        + 'execution2: ' + str(execution) + '\n'
                        + 'period: ' + str(period) + '\n'
                        + 'high_date: ' + str(low_date) + '\n'
                        + 'low_date: ' + str(high_date) + '\n'
                        + 'low_price: ' + str(high_price) + '\n'
                        + 'index: ' + str(index0))
        ...
        for idx, row in trade_seq.iterrows():
            if row['Type'] == 'short':
                buy_idx = idx
                buy_y = row['target']
            elif row['Type'] == 'buy' and buy_idx is not None:
                sell_idx = idx
                sell_y = row['target']
                ax2.plot(
