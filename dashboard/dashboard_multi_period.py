...
PRESET_RESULT_DIR = r"D:\Code\backtest-release\Backtest v2 ratio\result\xagusd_30s_all long_momentum outcome_fix_wd"
...
def _parse_summary(content: bytes, file_name: str, program_id: str | None = None) -> dict:
    ...
    if cw_col and pd.to_numeric(df[cw_col], errors="coerce").notna().any():
        fourth_field = "close_withdrawal_threshold"
    elif ow_col and pd.to_numeric(df[ow_col], errors="coerce").notna().any():
        fourth_field = "open_withdrawal_threshold"
    ...
    for idx, (_, row) in enumerate(df.iterrows()):
        ...
        records.append({
            ...
            "withdrawal_close_count": int(row[wd_close_col]) if wd_close_col and not pd.isna(row[wd_close_col]) else None,
            "speed_close_count": int(row[speed_close_col]) if speed_close_col and not pd.isna(row[speed_close_col]) else None,
        })
...
def _parse_price(content: bytes) -> dict:
    ...
    if not all([date_col, open_col, high_col, low_col, close_col]):
        raise ValueError("perf.xlsx 缺少 OHLC 列。")
...
def _parse_trans(content: bytes) -> dict:
    ...
    if not all([date_col, type_col, price_col]):
        raise ValueError("trans.xlsx 缺少交易列。")
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def _file(self):
        ...
        if not message.is_multipart():
            raise ValueError("请求内容不是 multipart/form-data。")
...
</style></head><body><div class="wrap"><div class="panel"><div class="top"><div><div class="title">Backtest Dashboard</div><div class="sub">总览读取 outcome_stats，详情切换读取 trans.xlsx，原 HTML 仍可随时打开。</div></div><div id="folderName" class="box">尚未选择结果大文件夹</div><select id="batchSelect" class="sel" disabled><option value="">请选择回测批次</option></select><button id="pickBtn" class="btn primary">选择结果文件夹</button></div><div style="margin-top:14px" class="tabs"><button id="tabOverview" class="tab active">主页面</button><button id="tabDetail" class="tab">详情页</button></div>
<div id="pageOverview" class="page active" style="margin-top:14px"><div class="chartbox"><div class="charttitle">收益总览</div><div id="overviewEmpty" class="empty">请选择一个带有 outcome_stats.xlsx 的结果大文件夹。</div><div id="overviewChart" class="chart" style="display:none"></div></div></div>
<div id="pageDetail" class="page" style="margin-top:14px"><div class="detail"><div class="left"><div class="chartbox"><div class="controls"><div><div class="label">窗口大小</div><input id="inputBar" class="num" type="number" disabled></div><div><div class="label">速度幅度限制</div><input id="inputThreshold" class="num" type="number" disabled></div><div><div class="label">持续门槛</div><input id="inputCont" class="num" type="number" disabled></div><button id="openHtml" class="btn" disabled>打开原 HTML</button></div></div><div class="chartbox"><div class="charttitle">公共行情图与买卖点</div><div id="priceEmpty" class="empty">请选择一个批次，再从主页面点选一个参数点。</div><div id="priceChart" class="chart" style="display:none"></div></div><div class="chartbox"><div class="charttitle">交易资金曲线</div><div id="capitalEmpty" class="empty">当前还没有可用的 trans.xlsx。</div><div id="capitalChart" class="chart" style="display:none;min-height:260px"></div></div><div id="htmlWrap" class="iframe"><iframe id="htmlFrame"></iframe></div></div>
<div class="right"><div class="chartbox"><div class="charttitle">当前参数</div><div class="meta"><div class="row"><div class="name">结果文件夹</div><div id="metaFolder">-</div></div><div class="row"><div class="name">批次</div><div id="metaBatch">-</div></div><div class="row"><div class="name">参数标签</div><div id="metaTag">-</div></div><div class="row"><div class="name">最终资本</div><div id="metaCapital">-</div></div><div class="row"><div class="name">交易次数</div><div id="metaTrade">-</div></div><div class="row"><div class="name">最大回撤</div><div id="metaWd">-</div></div><div class="row"><div class="name">峰值资本</div><div id="metaHigh">-</div></div></div></div><div class="chartbox"><div class="charttitle">状态</div><div id="status" class="empty" style="min-height:100px">等待目录</div></div></div></div></div></div></div><input id="folderInput" type="file" webkitdirectory directory multiple style="display:none">
<script>
...
async function setRecord(record,switchTab){state.activeRecord=record;inputsFrom(record);meta(record);htmlEnabled(record);$("htmlWrap").classList.remove("active");if(switchTab)setTab("detail");try{const [price,trans]=await Promise.all([ensurePrice(),ensureTrans(record)]);priceChart(price,trans);capitalChart(trans);setStatus("详情已同步")}catch(error){$("priceChart").style.display="none";$("capitalChart").style.display="none";$("priceEmpty").style.display="grid";$("capitalEmpty").style.display="grid";$("priceEmpty").innerHTML="详情载入失败：<br>"+esc(error.message);$("capitalEmpty").innerHTML="详情载入失败：<br>"+esc(error.message);setStatus("详情载入失败")}}
async function loadSummary(file){setStatus("正在读取 outcome_stats");state.summary=file.relative_path?await fetchJson("/api/preset-summary?path="+encodeURIComponent(file.relative_path)):await upload("/api/summary",file);configInput($("inputBar"),state.summary.controls.open_bar);configInput($("inputThreshold"),state.summary.controls.open_threshold);configInput($("inputCont"),state.summary.controls.open_continous_threshold);overview();const record=recordByKey(state.summary.default_key);if(record)await setRecord(record,false);setStatus("批次已载入")}
function folderChanged(files,folderLabel){resetUi();state.files=[...files];state.summaryFiles=state.files.filter(isSummary);state.transFiles=state.files.filter(isTrans);state.perfFiles=state.files.filter(isPerf);state.htmlFiles=state.files.filter(isHtml);if(!state.files.length){$("folderName").textContent="尚未选择结果大文件夹";setStatus("等待目录");return}state.folderLabel=folderLabel||(rel(state.files[0]).split("/")[0]||"已选择目录");$("folderName").textContent=state.folderLabel;$("metaFolder").textContent=state.folderLabel;const select=$("batchSelect");select.innerHTML='<option value="">请选择回测批次</option>';for(const file of [...state.summaryFiles].sort((a,b)=>fileMtime(b)-fileMtime(a))){const opt=document.createElement("option");opt.value=pathOf(file)||file.name;opt.textContent=file.name.replace(/\\.xlsx$/i,"");select.appendChild(opt)}select.disabled=!state.summaryFiles.length;if(!state.summaryFiles.length){setStatus("缺少 outcome_stats");$("overviewEmpty").innerHTML="当前目录里没有 outcome_stats.xlsx。<br>请使用更新后的回测程序重新输出 outcome stats 汇总文件。";return}setStatus("目录已载入");$("overviewEmpty").innerHTML="请选择一个回测批次。"}
$("pickBtn").addEventListener("click",()=>{$("folderInput").click()});$("folderInput").addEventListener("change",evt=>folderChanged(evt.target.files));$("batchSelect").addEventListener("change",async()=>{const file=state.summaryFiles.find(item=>(pathOf(item)||item.name)===$("batchSelect").value);if(!file)return;try{await loadSummary(file)}catch(error){$("overviewChart").style.display="none";$("overviewEmpty").style.display="grid";$("overviewEmpty").innerHTML="汇总文件读取失败：<br>"+esc(error.message);setStatus("批次读取失败")}});$("tabOverview").addEventListener("click",()=>setTab("overview"));$("tabDetail").addEventListener("click",()=>setTab("detail"));$("inputBar").addEventListener("change",async()=>{const record=recordByKey(selKey());if(record)await setRecord(record,false);else{state.activeRecord=null;meta(null);htmlEnabled(null);$("priceChart").style.display="none";$("capitalChart").style.display="none";$("priceEmpty").style.display="grid";$("capitalEmpty").style.display="grid";$("priceEmpty").innerHTML="当前三元参数组合没有对应结果。";$("capitalEmpty").innerHTML="当前三元参数组合没有对应 trans.xlsx。";setStatus("该参数组合没有结果")}});$("inputThreshold").addEventListener("change",async()=>{$("inputBar").dispatchEvent(new Event("change"))});$("inputCont").addEventListener("change",async()=>{$("inputBar").dispatchEvent(new Event("change"))});$("openHtml").addEventListener("click",()=>{if(!state.activeRecord||!state.summary)return;const file=detailFile(state.htmlFiles,state.summary.batch_label,state.activeRecord.param_tag,".html");if(!file)return;let url=file.relative_path?"/api/preset-html?path="+encodeURIComponent(file.relative_path):state.htmlUrlCache.get(state.activeRecord.selection_key);if(!file.relative_path&&!url){url=URL.createObjectURL(file);state.htmlUrlCache.set(state.activeRecord.selection_key,url)}$("htmlFrame").src=url;$("htmlWrap").classList.add("active")});async function bootstrapPreset(){try{const preset=await fetchJson("/api/preset-index");if(preset.error){$("folderName").textContent="预设目录无效";setStatus("预设目录读取失败：<br>"+esc(preset.error));return}if(!preset.enabled){return}folderChanged(preset.files||[],preset.folder_label||preset.folder_path||"预设目录");$("pickBtn").textContent="改选结果文件夹";if(state.summaryFiles.length===1){const first=state.summaryFiles[0];$("batchSelect").value=pathOf(first)||first.name;$("batchSelect").dispatchEvent(new Event("change"))}}catch(error){setStatus("预设目录读取失败：<br>"+esc(error.message))}}bootstrapPreset();
</script></body></html>"""
  ...
  <div class="panel">
    <div class="top">
      <div>
        ...
        <div class="sub">总览读取 outcome_stats，详情切换读取 trans.xlsx，原 HTML 仍可随时打开。</div>
      </div>
      <div id="folderName" class="box">尚未选择结果大文件夹</div>
      <select id="batchSelect" class="sel" disabled>
        <option value="">请选择回测批次</option>
      </select>
      <button id="pickBtn" class="btn primary">选择结果文件夹</button>
    </div>

    <div class="tabs">
      <button id="tabOverview" class="tab active">主页面</button>
      <button id="tabDetail" class="tab">详情页</button>
    </div>

    <div id="pageOverview" class="page active">
      <div class="chartbox">
        <div class="charttitle">收益总览</div>
        <div id="overviewEmpty" class="empty">请选择一个带有 outcome_stats.xlsx 的结果大文件夹。</div>
    ...
    <div id="pageDetail" class="page">
      <div class="detail-shell">
        <div class="chartbox price-box">
            <div class="charttitle">公共行情图与买卖点</div>
            <div class="chartsub">K 线采用原 HTML 的黑白风格，不显示没有数据的日期间隙。</div>
            <div id="priceEmpty" class="empty">请选择一个批次，再从主页面点选一个参数点。</div>
        ...
        <div class="detail-lower">
          <div class="info-stack">
            ...
            <div class="controls-grid">
              ...
              <div>
                <div class="label">速度限制</div>
                <input id="inputThreshold" class="num" type="number" disabled>
              </div>
              <div>
                <div class="label">开仓门槛</div>
                <input id="inputCont" class="num" type="number" disabled>
              </div>
              <div>
                <div class="label">原 HTML</div>
                <button id="openHtml" class="btn" disabled>打开原 HTML</button>
              </div>
            </div>
          </div>
            ...
            <div id="capitalEmpty" class="empty">当前还没有可用的 trans.xlsx。</div>
          ...
          <div class="drawer-head">
            <div class="drawer-title-wrap">
              <div class="charttitle" style="margin-bottom:4px">设置页</div>
              <div class="sub">当前参数、状态与原 HTML 预览。</div>
            </div>
          ...
          <div class="drawer-body">
            <div class="chartbox">
              ...
              <div class="meta">
                <div class="row"><div class="name">结果文件夹</div><div id="metaFolder">-</div></div>
                <div class="row"><div class="name">批次</div><div id="metaBatch">-</div></div>
                <div class="row"><div class="name">参数标签</div><div id="metaTag">-</div></div>
                <div class="row"><div class="name">最终资本</div><div id="metaCapital">-</div></div>
                <div class="row"><div class="name">交易次数</div><div id="metaTrade">-</div></div>
                <div class="row"><div class="name">最大回撤</div><div id="metaWd">-</div></div>
                <div class="row"><div class="name">峰值资本</div><div id="metaHigh">-</div></div>
              </div>
            </div>

            <div class="chartbox">
              <div class="charttitle">状态</div>
              <div id="status" class="empty" style="min-height:90px">等待目录</div>
            </div>

            <div class="chartbox">
              <div class="charttitle">原 HTML 预览</div>
              <div id="htmlEmpty" class="empty">选择一个参数后，可在这里预览对应的原 HTML。</div>
...
function resetUi(){for(const url of state.htmlUrlCache.values()){URL.revokeObjectURL(url)}state.summary=null;state.activeRecord=null;state.folderLabel="";state.priceCache.clear();state.transCache.clear();state.htmlUrlCache.clear();$("batchSelect").innerHTML='<option value="">请选择回测批次</option>';$("batchSelect").disabled=true;$("overviewChart").style.display="none";$("priceChart").style.display="none";$("capitalChart").style.display="none";$("overviewEmpty").style.display="grid";$("priceEmpty").style.display="grid";$("capitalEmpty").style.display="grid";$("openHtml").disabled=true;$("inputBar").disabled=true;$("inputThreshold").disabled=true;$("inputCont").disabled=true;clearHtmlPreview("选择一个参数后，可在这里预览对应的原 HTML。");$("metaFolder").textContent="-";$("metaBatch").textContent="-";$("metaTag").textContent="-";$("metaCapital").textContent="-";$("metaTrade").textContent="-";$("metaWd").textContent="-";$("metaHigh").textContent="-"}
function configInput(el,meta){const has=meta&&meta.values&&meta.values.length;el.disabled=!has;if(!has){el.value="";el.removeAttribute("min");el.removeAttribute("max");el.removeAttribute("step");return}el.min=meta.values[0];el.max=meta.values[meta.values.length-1];el.step=meta.step??"any"}
function batchFile(files,label,suffix){const arr=files.filter(file=>file.name.toLowerCase().endsWith(suffix)&&file.name.includes(label)).sort((a,b)=>fileMtime(b)-fileMtime(a));return arr[0]||null}
function detailFile(files,label,paramTag,suffix){const arr=files.filter(file=>file.name.toLowerCase().endsWith(suffix)&&file.name.includes(label)&&file.name.includes(paramTag)).sort((a,b)=>fileMtime(b)-fileMtime(a));return arr[0]||null}
function recordByKey(key){return state.summary?.records.find(item=>item.selection_key===key)||null}
...
async function setRecord(record,switchTab){state.activeRecord=record;inputsFrom(record);meta(record);htmlEnabled(record);clearHtmlPreview("选择一个参数后，可在这里预览对应的原 HTML。");if(switchTab)setTab("detail");try{const [price,trans]=await Promise.all([ensurePrice(),ensureTrans(record)]);priceChart(price,trans);capitalChart(trans);setStatus("详情已同步")}catch(error){$("priceChart").style.display="none";$("capitalChart").style.display="none";$("priceEmpty").style.display="grid";$("capitalEmpty").style.display="grid";$("priceEmpty").innerHTML="详情载入失败：<br>"+esc(error.message);$("capitalEmpty").innerHTML="详情载入失败：<br>"+esc(error.message);setStatus("详情载入失败")}}
async function loadSummary(file){setStatus("正在读取 outcome_stats");state.summary=file.relative_path?await fetchJson("/api/preset-summary?path="+encodeURIComponent(file.relative_path)):await upload("/api/summary",file);configInput($("inputBar"),state.summary.controls.open_bar);configInput($("inputThreshold"),state.summary.controls.open_threshold);configInput($("inputCont"),state.summary.controls.open_continous_threshold);overview();const record=recordByKey(state.summary.default_key);if(record)await setRecord(record,false);setStatus("批次已载入")}
function folderChanged(files,folderLabel){resetUi();state.files=[...files];state.summaryFiles=state.files.filter(isSummary);state.transFiles=state.files.filter(isTrans);state.perfFiles=state.files.filter(isPerf);state.htmlFiles=state.files.filter(isHtml);if(!state.files.length){$("folderName").textContent="尚未选择结果大文件夹";setStatus("等待目录");return}state.folderLabel=folderLabel||(rel(state.files[0]).split("/")[0]||"已选择目录");$("folderName").textContent=state.folderLabel;$("metaFolder").textContent=state.folderLabel;const select=$("batchSelect");select.innerHTML='<option value="">请选择回测批次</option>';for(const file of [...state.summaryFiles].sort((a,b)=>fileMtime(b)-fileMtime(a))){const opt=document.createElement("option");opt.value=pathOf(file)||file.name;opt.textContent=file.name.replace(/\\.xlsx$/i,"");select.appendChild(opt)}select.disabled=!state.summaryFiles.length;if(!state.summaryFiles.length){setStatus("缺少 outcome_stats");$("overviewEmpty").innerHTML="当前目录里没有 outcome_stats.xlsx。<br>请使用更新后的回测程序重新输出 outcome stats 汇总文件。";return}setStatus("目录已载入");$("overviewEmpty").innerHTML="请选择一个回测批次。"}

$("pickBtn").addEventListener("click",()=>{$("folderInput").click()});
$("folderInput").addEventListener("change",evt=>folderChanged(evt.target.files));
$("batchSelect").addEventListener("change",async()=>{const file=state.summaryFiles.find(item=>(pathOf(item)||item.name)===$("batchSelect").value);if(!file)return;try{await loadSummary(file)}catch(error){$("overviewChart").style.display="none";$("overviewEmpty").style.display="grid";$("overviewEmpty").innerHTML="汇总文件读取失败：<br>"+esc(error.message);setStatus("批次读取失败")}});
...
$("inputBar").addEventListener("change",async()=>{const record=recordByKey(selKey());if(record)await setRecord(record,false);else{state.activeRecord=null;meta(null);htmlEnabled(null);$("priceChart").style.display="none";$("capitalChart").style.display="none";$("priceEmpty").style.display="grid";$("capitalEmpty").style.display="grid";$("priceEmpty").innerHTML="当前三元参数组合没有对应结果。";$("capitalEmpty").innerHTML="当前三元参数组合没有对应 trans.xlsx。";clearHtmlPreview("当前三元参数组合没有对应的原 HTML。");setStatus("该参数组合没有结果")}});
...
$("openHtml").addEventListener("click",()=>{if(!state.activeRecord||!state.summary)return;const url=htmlUrlFor(state.activeRecord);if(!url){setStatus("当前参数缺少原 HTML");return}showHtmlPreview(url);setSettingsCollapsed(false);setStatus("原 HTML 已打开");try{const popup=window.open(url,"_blank","noopener");if(!popup){window.location.href=url}}catch(error){window.location.href=url}});

async function bootstrapPreset(){try{const preset=await fetchJson("/api/preset-index");if(preset.error){$("folderName").textContent="预设目录无效";setStatus("预设目录读取失败：<br>"+esc(preset.error));return}if(!preset.enabled){return}folderChanged(preset.files||[],preset.folder_label||preset.folder_path||"预设目录");$("pickBtn").textContent="改选结果文件夹";if(state.summaryFiles.length){const first=state.summaryFiles[0];$("batchSelect").value=pathOf(first)||first.name;$("batchSelect").dispatchEvent(new Event("change"))}}catch(error){setStatus("预设目录读取失败：<br>"+esc(error.message))}}
...
setSettingsCollapsed=function(collapsed){state.settingsCollapsed=collapsed;const drawer=$("settingsDrawer");const toggle=$("settingsToggle");if(!drawer||!toggle)return;drawer.classList.toggle("collapsed",collapsed);toggle.textContent=collapsed?"展开":"收起";toggle.title=collapsed?"展开边栏":"收起边栏"}
...
.drawer-toggle::before{content:"◀";font-size:14px;line-height:1;color:rgba(21,32,51,.72)}
.drawer.collapsed .drawer-toggle::before{content:"▶"}
...
PAGE = PAGE.replace(
    'showHtmlPreview(url);setSettingsCollapsed(false);setStatus("原 HTML 已打开");try{const popup=window.open(url,"_blank","noopener");if(!popup){window.location.href=url}}catch(error){window.location.href=url}',
    'const popup=window.open(url,"_blank","noopener,noreferrer");if(popup&&popup.focus){popup.focus()}setStatus(popup?"原 HTML 已在新窗口打开":"浏览器拦截了新窗口")'
)
PAGE = PAGE.replace(
    '$("pickBtn").textContent="改选结果文件夹";',
    '$("pickBtn").textContent="修改目标文件夹";'
)
PAGE = PAGE.replace(
    'function keyOf(v){if(v===null||v===undefined||v==="")return"";const n=Number(v);if(Number.isNaN(n))return"";return String(Number(n.toFixed(10))).replace(/\\.0+$/,"")}',
...
PAGE = PAGE.replace(
    'const state={files:[],summaryFiles:[],transFiles:[],perfFiles:[],htmlFiles:[],summary:null,activeRecord:null,folderLabel:"",priceCache:new Map(),transCache:new Map(),htmlUrlCache:new Map(),settingsCollapsed:false};',
    'const state={files:[],summaryFiles:[],transFiles:[],perfFiles:[],htmlFiles:[],summary:null,activeRecord:null,folderLabel:"",priceCache:new Map(),transCache:new Map(),htmlUrlCache:new Map(),settingsCollapsed:false,showCapitalOverlay:true,showOverview3d:false};try{const savedCapitalOverlay=localStorage.getItem("dashboard_show_capital_overlay");if(savedCapitalOverlay!==null){state.showCapitalOverlay=savedCapitalOverlay==="1"}const savedOverview3d=localStorage.getItem("dashboard_show_overview_3d");if(savedOverview3d!==null){state.showOverview3d=savedOverview3d==="1"}}catch(error){}'
)
PAGE = PAGE.replace(
    'function setStatus(text){const node=$("status");if(node){node.innerHTML=text}}',
    'function setStatus(text){const node=$("status");if(node){node.innerHTML=text}}function syncCapitalOverlayToggle(){const node=$("toggleCapitalOverlay");if(node){node.checked=!!state.showCapitalOverlay}}function saveCapitalOverlay(value){state.showCapitalOverlay=!!value;syncCapitalOverlayToggle();try{localStorage.setItem("dashboard_show_capital_overlay",state.showCapitalOverlay?"1":"0")}catch(error){}}function syncOverview3dToggle(){const node=$("toggleOverview3d");if(node){node.checked=!!state.showOverview3d}}function saveOverview3d(value){state.showOverview3d=!!value;syncOverview3dToggle();try{localStorage.setItem("dashboard_show_overview_3d",state.showOverview3d?"1":"0")}catch(error){}}'
)
PAGE = PAGE.replace(
    '#overviewChart.chart{min-height:510px}',
...
function currentPriceTitle(){const product=String(state.folderLabel||"").replace(/\\s+long_momentum(?:_atr|_ratio)? outcome$/i,"").trim();const match=String(state.summary?.batch_label||"").match(/\\d{8}-\\d{8}/);const dateText=match?match[0]:"";return[product,dateText].filter(Boolean).join(" ")||"品种 日期"}
...
PAGE = PAGE.replace(
    '<div id="overviewChart" class="chart" style="display:none"></div>\n      </div>',
    '<div id="overviewChart" class="chart" style="display:none"></div>\n      </div>\n      <div class="chartbox" style="margin-top:16px">\n        <div class="overview-3d-head"><div class="charttitle">参数三维图</div><label class="overview3d-toggle"><input id="toggleOverview3d" type="checkbox"><span>显示</span></label></div>\n        <div id="overview3dEmpty" class="empty">请选择一个回测批次。</div>\n        <div id="overview3dChart" class="chart" style="display:none"></div>\n      </div>'
)
PAGE = PAGE.replace(
    "</style>",
    """
#overview3dChart.chart{min-height:620px}
.overview-3d-head{display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap}
.overview3d-toggle{display:flex;align-items:center;gap:8px;height:32px;padding:0 12px;border:1px solid #d9e1ee;border-radius:999px;background:#fff;color:#152033;font-size:13px;box-sizing:border-box}
.overview3d-toggle input{margin:0}
</style>"""
)
PAGE = PAGE.replace(
    'setupLayout();\nsetSettingsCollapsed(false);\nbootstrapPreset();',
...
function renderOverview3d(){const chart=$("overview3dChart");const empty=$("overview3dEmpty");if(!chart||!empty){return}if(!state.summary||!state.summary.records.length){setOverview3dEmpty("请选择一个回测批次。");return}const rows=state.summary.records.filter(row=>row.capital!==null&&row.capital!==undefined&&row.open_bar!==null&&row.open_bar!==undefined);if(!rows.length){setOverview3dEmpty("当前批次缺少可用于三维图的参数数据。");return}const order=rows.map((_,index)=>index+1);empty.style.display="none";chart.style.display="block";const text=rows.map((row,index)=>["顺序: "+(index+1),"param_tag: "+esc(row.param_tag),"时间窗口: "+formatParamDisplay(row.open_bar),"速度限制: "+formatParamDisplay(row.open_threshold),"开仓门槛: "+formatParamDisplay(row.open_continous_threshold),"capital: "+formatMetric3(row.capital),"trade_num: "+(row.trade_num??"-")].join("<br>"));Plotly.newPlot(chart,[{type:"scatter3d",mode:"lines+markers",x:order,y:rows.map(row=>row.capital),z:rows.map(row=>row.open_bar),customdata:rows.map(row=>row.selection_key),text:text,hovertemplate:"%{text}<extra></extra>",line:{color:"#2f6bff",width:3},marker:{size:5,color:rows.map(row=>row.capital),colorscale:"Viridis",opacity:0.95,colorbar:{title:"capital"}}}],{margin:{l:0,r:0,t:10,b:0},paper_bgcolor:"rgba(0,0,0,0)",scene:{xaxis:{title:"参数组合顺序",gridcolor:"#e8eef8",zerolinecolor:"#e8eef8",backgroundcolor:"#fff"},yaxis:{title:"capital",gridcolor:"#e8eef8",zerolinecolor:"#e8eef8",backgroundcolor:"#fff"},zaxis:{title:"时间窗口",gridcolor:"#e8eef8",zerolinecolor:"#e8eef8",backgroundcolor:"#fff"}},showlegend:false},{responsive:true,displayModeBar:false});if(chart.removeAllListeners){chart.removeAllListeners("plotly_click")}chart.on("plotly_click",evt=>{const key=evt?.points?.[0]?.customdata;const record=recordByKey(key);if(record){setRecord(record,true)}})}
const overviewBase3d=overview;overview=function(){overviewBase3d();renderOverview3d()}
const resetUiBase3d=resetUi;resetUi=function(){resetUiBase3d();setOverview3dEmpty("请选择一个带有 outcome_stats.xlsx 的结果大文件夹。")}
const folderChangedBase3d=folderChanged;folderChanged=function(files,folderLabel){const result=folderChangedBase3d(files,folderLabel);if(state.summaryFiles?.length){setOverview3dEmpty("请选择一个回测批次。")}else{setOverview3dEmpty("当前目录里没有 outcome_stats.xlsx。<br>请使用更新后的回测程序重新输出 outcome stats 汇总文件。")}return result}
setupLayout();
setSettingsCollapsed(false);
bootstrapPreset();'''
)
...
PAGE = PAGE.replace(
    ...
    'function renderOverview3d(){const chart=$("overview3dChart");const empty=$("overview3dEmpty");if(!chart||!empty){return}if(!state.summary||!state.summary.records.length){setOverview3dEmpty("请选择一个回测批次。");return}const isFlatCapital=value=>{const n=Number(value);return Number.isFinite(n)&&Math.abs(n-100)<1e-9};const rows=state.summary.records.filter(row=>row.capital!==null&&row.capital!==undefined&&row.open_bar!==null&&row.open_bar!==undefined&&row.open_threshold!==null&&row.open_threshold!==undefined&&!isFlatCapital(row.capital));if(!rows.length){setOverview3dEmpty("当前批次缺少可用于三维图的参数数据。");return}empty.style.display="none";chart.style.display="block";const text=rows.map(row=>["param_tag: "+esc(row.param_tag),"时间窗口: "+formatParamDisplay(row.open_bar),"速度限制: "+formatParamDisplay(row.open_threshold),"开仓门槛: "+formatParamDisplay(row.open_continous_threshold),"capital: "+formatMetric3(row.capital),"trade_num: "+(row.trade_num??"-")].join("<br>"));Plotly.newPlot(chart,[{type:"scatter3d",mode:"markers",x:rows.map(row=>row.open_threshold),y:rows.map(row=>row.capital),z:rows.map(row=>row.open_bar),customdata:rows.map(row=>row.selection_key),text:text,hovertemplate:"%{text}<extra></extra>",marker:{size:5,color:rows.map(row=>row.capital),colorscale:"Viridis",opacity:0.95,colorbar:{title:"capital"}}}],{margin:{l:0,r:0,t:10,b:0},paper_bgcolor:"rgba(0,0,0,0)",scene:{xaxis:{title:"速度限制",gridcolor:"#e8eef8",zerolinecolor:"#e8eef8",backgroundcolor:"#fff"},yaxis:{title:"capital",gridcolor:"#e8eef8",zerolinecolor:"#e8eef8",backgroundcolor:"#fff"},zaxis:{title:"时间窗口",gridcolor:"#e8eef8",zerolinecolor:"#e8eef8",backgroundcolor:"#fff"}},showlegend:false},{responsive:true,displayModeBar:false});if(chart.removeAllListeners){chart.removeAllListeners("plotly_click")}chart.on("plotly_click",evt=>{const key=evt?.points?.[0]?.customdata;const record=recordByKey(key);if(record){setRecord(record,true)}})}'
)


PAGE = PAGE.replace(
    'const text=rows.map(row=>["param_tag: "+esc(row.param_tag),"时间窗口: "+formatParamDisplay(row.open_bar),"速度限制: "+formatParamDisplay(row.open_threshold),"开仓门槛: "+formatParamDisplay(row.open_continous_threshold),"capital: "+formatMetric3(row.capital),"trade_num: "+(row.trade_num??"-")].join("<br>"));Plotly.newPlot(chart,[{type:"scatter3d",mode:"markers",x:rows.map(row=>row.open_threshold),y:rows.map(row=>row.capital),z:rows.map(row=>row.open_bar),customdata:rows.map(row=>row.selection_key),text:text,hovertemplate:"%{text}<extra></extra>",marker:{size:5,color:rows.map(row=>row.capital),colorscale:"Viridis",opacity:0.95,colorbar:{title:"capital"}}}],{margin:{l:0,r:0,t:10,b:0},paper_bgcolor:"rgba(0,0,0,0)",scene:{xaxis:{title:"速度限制",gridcolor:"#e8eef8",zerolinecolor:"#e8eef8",backgroundcolor:"#fff"},yaxis:{title:"capital",gridcolor:"#e8eef8",zerolinecolor:"#e8eef8",backgroundcolor:"#fff"},zaxis:{title:"时间窗口",gridcolor:"#e8eef8",zerolinecolor:"#e8eef8",backgroundcolor:"#fff"}},showlegend:false},{responsive:true,displayModeBar:false});',
    'const xValues=rows.map(row=>Number(row.open_threshold)).filter(Number.isFinite);const zValues=rows.map(row=>Number(row.capital)).filter(Number.isFinite);const timeWindows=[...new Set(rows.map(row=>Number(row.open_bar)).filter(Number.isFinite))].sort((a,b)=>a-b);const separatorYs=[];for(let index=1;index<timeWindows.length;index+=1){separatorYs.push((timeWindows[index-1]+timeWindows[index])/2)}const xMin=xValues.length?Math.min(...xValues):0;const xMax=xValues.length?Math.max(...xValues):1;const zMin=zValues.length?Math.min(...zValues):0;const zMax=zValues.length?Math.max(...zValues):1;const text=rows.map(row=>["param_tag: "+esc(row.param_tag),"时间窗口: "+formatParamDisplay(row.open_bar),"速度限制: "+formatParamDisplay(row.open_threshold),"开仓门槛: "+formatParamDisplay(row.open_continous_threshold),"capital: "+formatMetric3(row.capital),"trade_num: "+(row.trade_num??"-")].join("<br>"));const planeTraces=separatorYs.map(value=>({type:"surface",x:[[xMin,xMax],[xMin,xMax]],y:[[value,value],[value,value]],z:[[zMin,zMin],[zMax,zMax]],surfacecolor:[[0,0],[0,0]],colorscale:[[0,"rgba(120,120,120,0.1)"],[1,"rgba(120,120,120,0.1)"]],showscale:false,opacity:0.1,hoverinfo:"skip"}));Plotly.newPlot(chart,[...planeTraces,{type:"scatter3d",mode:"markers",x:rows.map(row=>row.open_threshold),y:rows.map(row=>row.open_bar),z:rows.map(row=>row.capital),customdata:rows.map(row=>row.selection_key),text:text,hovertemplate:"%{text}<extra></extra>",marker:{size:5,color:rows.map(row=>row.capital),colorscale:"Viridis",opacity:0.95,colorbar:{title:"capital"}}}],{margin:{l:0,r:0,t:10,b:0},paper_bgcolor:"rgba(0,0,0,0)",scene:{xaxis:{title:"速度限制",gridcolor:"#e8eef8",zerolinecolor:"#e8eef8",backgroundcolor:"#fff"},yaxis:{title:"时间窗口",gridcolor:"#e8eef8",zerolinecolor:"#e8eef8",backgroundcolor:"#fff"},zaxis:{title:"capital",gridcolor:"#e8eef8",zerolinecolor:"#e8eef8",backgroundcolor:"#fff"}},showlegend:false},{responsive:true,displayModeBar:false});'
)
...
PAGE = PAGE.replace(
    'scene:{xaxis:{title:"速度限制",gridcolor:"#e8eef8",zerolinecolor:"#e8eef8",backgroundcolor:"#fff"},yaxis:{title:"时间窗口",gridcolor:"#e8eef8",zerolinecolor:"#e8eef8",backgroundcolor:"#fff"},zaxis:{title:"capital",gridcolor:"#e8eef8",zerolinecolor:"#e8eef8",backgroundcolor:"#fff"}}',
    'scene:{xaxis:{title:"时间窗口",gridcolor:"#e8eef8",zerolinecolor:"#e8eef8",backgroundcolor:"#fff"},yaxis:{title:"速度限制",gridcolor:"#e8eef8",zerolinecolor:"#e8eef8",backgroundcolor:"#fff"},zaxis:{title:"capital",gridcolor:"#e8eef8",zerolinecolor:"#e8eef8",backgroundcolor:"#fff"}}'
)
...
PAGE = PAGE.replace(
    '<div class="charttitle">收益总览</div>\n        <div id="overviewEmpty" class="empty">请选择一个带有 outcome_stats.xlsx 的结果大文件夹。</div>',
    '<div class="overview-head"><div class="charttitle">收益总览</div><div class="overview-sort-wrap"><span class="overview-sort-label">排序</span><select id="overviewSortMode" class="sel overview-sort-select"><option value="current">当前序号</option><option value="capital">收益排序</option></select></div></div>\n        <div id="overviewEmpty" class="empty">请选择一个带有 outcome_stats.xlsx 的结果大文件夹。</div>'
)
PAGE = PAGE.replace(
    '.price-actions #openHtml{min-width:160px}.price-actions #backOverviewBtn{min-width:112px;display:inline-flex;align-items:center;justify-content:center;padding:0 16px}',
...
PAGE = PAGE.replace(
    ...
    '<option value="current">序号</option><option value="capital">收益</option><option value="drawdown">最小回撤</option><option value="avg_profit">平均收益</option><option value="trade_num">交易次数</option></select>'
)
PAGE = PAGE.replace(
...
PAGE = PAGE.replace(
    ...
    'xaxis:sortMode==="capital"?{title:"收益排名",showgrid:false,zeroline:false}:sortMode==="drawdown"?{title:"最小回撤排名",showgrid:false,zeroline:false}:{title:"om",showgrid:false,zeroline:false,tickmode:"array",tickvals:groupTicks.map(item=>item.value),ticktext:groupTicks.map(item=>item.label)}'
)
...
PAGE = PAGE.replace(
    ...
    '.overview-sort-select{min-width:148px}.price-head{display:flex;align-items:flex-start;justify-content:space-between;gap:16px;flex-wrap:wrap;margin-bottom:10px}.price-head .charttitle{margin-bottom:0}.price-head-stats{display:grid;gap:6px;margin-left:auto;justify-items:end}.price-head-stats-row{display:flex;align-items:center;justify-content:flex-end;gap:14px;flex-wrap:wrap}.price-head-stat{display:flex;align-items:center;gap:6px;font-size:12px;color:#66758c;white-space:nowrap}.price-head-stat strong{font-size:13px;color:#152033;font-weight:700}'
)
PAGE = PAGE.replace(
    'bootstrapPreset();\n</script>',
    '''function formatMetric2(value){if(value===null||value===undefined||value==="")return"-";const n=Number(value);if(Number.isNaN(n))return String(value);if(Number.isInteger(n))return String(n);return n.toFixed(2)}
function capitalProfit(record){const value=Number(record?.capital);return Number.isFinite(value)?value-100:null}
function avgProfitPerTrade(record){const profit=capitalProfit(record);const tradeCount=Number(record?.trade_num);if(profit===null||!Number.isFinite(tradeCount)||tradeCount<=0)return null;return profit/tradeCount}
function syncPriceHeaderStats(record){const tradeNode=$("priceHeadTrade");const wdNode=$("priceHeadWd");const avgNode=$("priceHeadAvgProfit");const finalNode=$("priceHeadFinalProfit");const wdCloseNode=$("priceHeadWdClose");const speedCloseNode=$("priceHeadSpeedClose");if(tradeNode){tradeNode.textContent=formatMetric2(record?.trade_num)}if(wdNode){wdNode.textContent=formatMetric2(record?.biggest_wd)}if(avgNode){avgNode.textContent=formatMetric2(avgProfitPerTrade(record))}if(finalNode){finalNode.textContent=formatMetric2(capitalProfit(record))}if(wdCloseNode){wdCloseNode.textContent=formatMetric2(record?.withdrawal_close_count)}if(speedCloseNode){speedCloseNode.textContent=formatMetric2(record?.speed_close_count)}}
const baseUpdatePriceHeaderForStats=updatePriceHeader;
updatePriceHeader=function(){baseUpdatePriceHeaderForStats();const priceBox=document.querySelector("#pageDetail .price-box");if(!priceBox)return;let head=$("priceHead");if(!head){head=document.createElement("div");head.id="priceHead";head.className="price-head";const title=document.querySelector("#pageDetail .price-box .charttitle");const stats=document.createElement("div");stats.className="price-head-stats";stats.innerHTML='<div class="price-head-stats-row"><div class="price-head-stat"><span>交易次数</span><strong id="priceHeadTrade">-</strong></div><div class="price-head-stat"><span>最大回撤</span><strong id="priceHeadWd">-</strong></div></div><div class="price-head-stats-row"><div class="price-head-stat"><span>平均收益</span><strong id="priceHeadAvgProfit">-</strong></div><div class="price-head-stat"><span>最终收益</span><strong id="priceHeadFinalProfit">-</strong></div></div><div class="price-head-stats-row"><div class="price-head-stat"><span>回撤平仓</span><strong id="priceHeadWdClose">-</strong></div><div class="price-head-stat"><span>速度平仓</span><strong id="priceHeadSpeedClose">-</strong></div></div>';if(title){head.appendChild(title)}head.appendChild(stats);priceBox.insertBefore(head,priceBox.firstChild)}const titleNode=head.querySelector(".charttitle");if(titleNode){titleNode.textContent=currentPriceTitle()}syncPriceHeaderStats(state.activeRecord)}
...
function detectProgramIdByFiles(files,folderLabel){const folderText=String(folderLabel||"").trim().toLowerCase();const labelHit=detectProgramIdFromText(folderLabel);if(labelHit)return labelHit;if(folderText.endsWith(" long outcome"))return "classic";const joined=(files||[]).slice(0,120).map(file=>String(file.name||"")+" "+String(pathOf(file)||"")).join("\\n");const fileHit=detectProgramIdFromText(joined);if(fileHit)return fileHit;return files&&files.length?"classic":""}
...
function programTagFromProgramId(programId){const lowered=String(programId||"").trim().toLowerCase();if(lowered==="ratio")return "long_momentum_ratio";if(lowered==="classic_atr")return "long_momentum_ATR";if(lowered==="garch")return "long_momentum_GARCH";return "long_momentum"}
...
function syncWithdrawalUiState(){const wdInput=$("inputWd");if(!wdInput){return}const label=wdInput.parentElement?.querySelector(".label");if(label){label.textContent="回撤限制"}wdInput.disabled=false;syncParamUi(wdInput)}
const readParamSelectionsLinkedWithdrawal=readParamSelections;readParamSelections=function(){return readParamSelectionsLinkedWithdrawal()}
const filterParamRowsLinkedWithdrawal=filterParamRows;filterParamRows=function(selections,ignoreField){return filterParamRowsLinkedWithdrawal(selections,ignoreField)}
const syncAvailableParamsLinkedWithdrawal=syncAvailableParams;syncAvailableParams=function(){const record=syncAvailableParamsLinkedWithdrawal();syncWithdrawalUiState();return record}
const inputsFromLinkedWithdrawal=inputsFrom;inputsFrom=function(record){inputsFromLinkedWithdrawal(record);syncWithdrawalUiState()}
...
const overviewProgramSpecificFinal=overview;overview=function(){if(!state.summary||!state.summary.records.length){return overviewProgramSpecificFinal()}const sortMode=$("overviewSortMode")?.value||"current";const numOrInf=value=>{const n=Number(value);return Number.isFinite(n)?n:Infinity};const numOrNegInf=value=>{const n=Number(value);return Number.isFinite(n)?n:-Infinity};const avgProfitValue=row=>{const capital=Number(row?.capital);const tradeNum=Number(row?.trade_num);if(!Number.isFinite(capital)||!Number.isFinite(tradeNum)||tradeNum<=0){return -Infinity}return (capital-100)/tradeNum};const tradeCountValue=row=>{const tradeNum=Number(row?.trade_num);return Number.isFinite(tradeNum)?tradeNum:-Infinity};const avgProfitText=row=>{const value=avgProfitValue(row);return Number.isFinite(value)?formatMetric3(value):"-"};const isFlatCapital=value=>{const n=Number(value);return Number.isFinite(n)&&Math.abs(n-100)<1e-9};const baseRows=[...state.summary.records].filter(row=>!isFlatCapital(row.capital)).sort((a,b)=>numOrInf(a.open_bar)-numOrInf(b.open_bar)||numOrInf(a.open_threshold)-numOrInf(b.open_threshold)||numOrInf(a.open_continous_threshold)-numOrInf(b.open_continous_threshold)||String(a.param_tag||"").localeCompare(String(b.param_tag||""))).map((row,index)=>({...row,__currentOrder:index+1}));const rows=sortMode==="capital"?[...baseRows].sort((a,b)=>numOrNegInf(b.capital)-numOrNegInf(a.capital)||a.__currentOrder-b.__currentOrder):sortMode==="drawdown"?[...baseRows].sort((a,b)=>numOrInf(a.biggest_wd)-numOrInf(b.biggest_wd)||a.__currentOrder-b.__currentOrder):sortMode==="avg_profit"?[...baseRows].sort((a,b)=>avgProfitValue(b)-avgProfitValue(a)||a.__currentOrder-b.__currentOrder):sortMode==="trade_num"?[...baseRows].sort((a,b)=>tradeCountValue(b)-tradeCountValue(a)||a.__currentOrder-b.__currentOrder):baseRows;const groupTicks=[];const separators=[];if(sortMode==="current"){let groupStart=0;for(let i=1;i<=rows.length;i+=1){const sameGroup=i<rows.length&&keyOf(rows[i]?.open_bar)===keyOf(rows[groupStart]?.open_bar);if(sameGroup)continue;const left=groupStart+1;const right=i;groupTicks.push({value:(left+right)/2,label:formatParamDisplay(rows[groupStart]?.open_bar)});if(i<rows.length){separators.push({type:"line",xref:"x",yref:"paper",x0:i+0.5,x1:i+0.5,y0:0,y1:1,line:{color:"rgba(120,120,120,0.1)",width:1}})}groupStart=i}}$("overviewEmpty").style.display="none";$("overviewChart").style.display="block";Plotly.newPlot($("overviewChart"),[{x:rows.map((_,i)=>i+1),y:rows.map(r=>r.capital),mode:"markers",marker:{size:6,color:"#2f6bff"},customdata:rows.map(r=>r.selection_key),text:rows.map((r,index)=>["param_tag: "+esc(r.param_tag),"序号: "+(r.__currentOrder??"-"),"收益排序: "+(sortMode==="capital"?index+1:"-"),"最小回撤排序: "+(sortMode==="drawdown"?index+1:"-"),"平均收益排序: "+(sortMode==="avg_profit"?index+1:"-"),"交易次数排序: "+(sortMode==="trade_num"?index+1:"-"),labelTextFinal("open_bar")+": "+formatParamDisplay(r.open_bar),labelTextFinal("open_threshold")+": "+formatParamDisplay(r.open_threshold),labelTextFinal("open_continous_threshold")+": "+formatParamDisplay(r.open_continous_threshold),"capital: "+(r.capital??"-"),"biggest_wd: "+formatMetric3(r.biggest_wd),"avg_profit: "+avgProfitText(r),"trade_num: "+(r.trade_num??"-")].join("<br>")),hovertemplate:"%{text}<extra></extra>",name:"capital"}],{margin:{l:44,r:18,t:16,b:40},paper_bgcolor:"rgba(0,0,0,0)",plot_bgcolor:"#fff",xaxis:sortMode==="capital"?{title:"收益排名",showgrid:false,zeroline:false}:sortMode==="drawdown"?{title:"最小回撤排名",showgrid:false,zeroline:false}:sortMode==="avg_profit"?{title:"平均收益排名",showgrid:false,zeroline:false}:sortMode==="trade_num"?{title:"交易次数排名",showgrid:false,zeroline:false}:{title:labelTextFinal("open_bar"),showgrid:false,zeroline:false,tickmode:"array",tickvals:groupTicks.map(item=>item.value),ticktext:groupTicks.map(item=>item.label)},yaxis:{title:"capital",showgrid:false,zeroline:false},shapes:separators,showlegend:false},{responsive:true,displayModeBar:false});if($("overviewChart").removeAllListeners){$("overviewChart").removeAllListeners("plotly_click")}$("overviewChart").on("plotly_click",evt=>{const key=evt?.points?.[0]?.customdata;const record=recordByKey(key);if(record)setRecord(record,true)})}
renderOverview3d=function(){const chart=$("overview3dChart");const empty=$("overview3dEmpty");if(!chart||!empty){return}syncOverview3dToggle();if(!state.showOverview3d){setOverview3dEmpty("三维图已关闭。");return}if(!state.summary||!state.summary.records.length){setOverview3dEmpty("请选择一个回测批次。");return}const isFlatCapital=value=>{const n=Number(value);return Number.isFinite(n)&&Math.abs(n-100)<1e-9};const rows=state.summary.records.filter(row=>row.capital!==null&&row.capital!==undefined&&row.open_bar!==null&&row.open_bar!==undefined&&row.open_threshold!==null&&row.open_threshold!==undefined&&!isFlatCapital(row.capital));if(!rows.length){setOverview3dEmpty("当前批次缺少可用于三维图的参数数据。");return}empty.style.display="none";chart.style.display="block";const text=rows.map(row=>["param_tag: "+esc(row.param_tag),labelTextFinal("open_bar")+": "+formatParamDisplay(row.open_bar),labelTextFinal("open_threshold")+": "+formatParamDisplay(row.open_threshold),labelTextFinal("open_continous_threshold")+": "+formatParamDisplay(row.open_continous_threshold),"capital: "+formatMetric3(row.capital),"trade_num: "+(row.trade_num??"-")].join("<br>"));Plotly.newPlot(chart,[{type:"scatter3d",mode:"markers",x:rows.map(row=>row.open_threshold),y:rows.map(row=>row.capital),z:rows.map(row=>row.open_bar),customdata:rows.map(row=>row.selection_key),text:text,hovertemplate:"%{text}<extra></extra>",marker:{size:5,color:rows.map(row=>row.capital),colorscale:"Viridis",opacity:0.95,colorbar:{title:"capital"}}}],{margin:{l:0,r:0,t:10,b:0},paper_bgcolor:"rgba(0,0,0,0)",scene:{xaxis:{title:labelTextFinal("open_threshold"),gridcolor:"#e8eef8",zerolinecolor:"#e8eef8",backgroundcolor:"#fff"},yaxis:{title:"capital",gridcolor:"#e8eef8",zerolinecolor:"#e8eef8",backgroundcolor:"#fff"},zaxis:{title:labelTextFinal("open_bar"),gridcolor:"#e8eef8",zerolinecolor:"#e8eef8",backgroundcolor:"#fff"}},showlegend:false},{responsive:true,displayModeBar:false});if(chart.removeAllListeners){chart.removeAllListeners("plotly_click")}chart.on("plotly_click",evt=>{const key=evt?.points?.[0]?.customdata;const record=recordByKey(key);if(record){setRecord(record,true)}})}
function bindOverview3dToggleFinal(){const toggle=$("toggleOverview3d");if(!toggle||toggle.dataset.bound){syncOverview3dToggle();return}toggle.dataset.bound="1";toggle.addEventListener("change",()=>{saveOverview3d(toggle.checked);renderOverview3d()});syncOverview3dToggle()}
const syncProgramSpecificLabelsFinal=syncProgramSpecificLabels;syncProgramSpecificLabels=function(){syncProgramSpecificLabelsFinal();syncOverview3dToggle();if($("overviewChart")?.style.display==="block"){overview()}if(state.showOverview3d&&$("overview3dChart")?.style.display==="block"){renderOverview3d()}}
setupLayout();
syncProgramSpecificLabels();
bindOverview3dToggleFinal();
syncOverview3dToggle();
bootstrapPreset();
</script>'''
)
...
PAGE = PAGE.replace(
    "</style>",
    """
.controls-card{width:100%;max-width:none}
.controls-grid{grid-template-columns:repeat(5,minmax(0,1fr))}
.time-range-host{display:grid;gap:6px;min-width:0}
.time-range-row{display:grid;grid-template-columns:minmax(0,1fr) 18px minmax(0,1fr);gap:8px;align-items:center}
.time-range-row .sel{width:100%;box-sizing:border-box;min-width:0}
.time-range-dash{color:#66758c;text-align:center;font-weight:600}
...
PAGE = PAGE.replace(
    'bootstrapPreset();\n</script>',
    '''state.timeFilterStart=state.timeFilterStart||"";
state.timeFilterEnd=state.timeFilterEnd||"";
state.timeFilterStartOptions=state.timeFilterStartOptions||[];
state.timeFilterEndOptions=state.timeFilterEndOptions||[];
function parseDashboardTime(value){const text=String(value||"").trim();if(!text)return NaN;return Date.parse(text.replace(" ","T"))}
function formatTimeFilterLabel(value){const text=String(value||"").trim();const match=text.match(/^\\d{4}-(\\d{2})-(\\d{2})[ T](\\d{2})/);if(match){return match[1]+"-"+match[2]+" "+match[3]}return text}
function buildTimeFilterOptions(values){const list=[...(values||[])].map(value=>String(value||"").trim()).filter(Boolean);const startOptions=[];const endMap=new Map();const startSeen=new Set();for(const value of list){const day=value.slice(0,10);if(!startSeen.has(day)){startSeen.add(day);startOptions.push(value)}endMap.set(day,value)}return{startOptions:startOptions,endOptions:[...endMap.values()]}}
function fillTimeFilterSelect(node,values){if(!node)return;node.innerHTML="";const allOption=document.createElement("option");allOption.value="";allOption.textContent="全部";node.appendChild(allOption);for(const value of values){const option=document.createElement("option");option.value=value;option.textContent=formatTimeFilterLabel(value);node.appendChild(option)}node.disabled=!values.length}
function syncTimeFilterControls(values){ensureTimeFilterControl();const startNode=$("inputTimeStart");const endNode=$("inputTimeEnd");if(!startNode||!endNode)return;const options=buildTimeFilterOptions(values);state.timeFilterStartOptions=options.startOptions;state.timeFilterEndOptions=options.endOptions;fillTimeFilterSelect(startNode,options.startOptions);fillTimeFilterSelect(endNode,options.endOptions);let nextStart=String(state.timeFilterStart||"");let nextEnd=String(state.timeFilterEnd||"");if(nextStart&&!options.startOptions.includes(nextStart)){nextStart=""}if(nextEnd&&!options.endOptions.includes(nextEnd)){nextEnd=""}if(nextStart&&nextEnd&&parseDashboardTime(nextStart)>parseDashboardTime(nextEnd)){nextEnd=""}state.timeFilterStart=nextStart;state.timeFilterEnd=nextEnd;startNode.value=nextStart;endNode.value=nextEnd}
function currentTimeFilterRange(){const start=String($("inputTimeStart")?.value||state.timeFilterStart||"");const end=String($("inputTimeEnd")?.value||state.timeFilterEnd||"");return{start:start,end:end,startMs:parseDashboardTime(start),endMs:parseDashboardTime(end)}}
function timeInDashboardRange(value,range){const ms=parseDashboardTime(value);if(!Number.isFinite(ms))return false;if(Number.isFinite(range.startMs)&&ms<range.startMs)return false;if(Number.isFinite(range.endMs)&&ms>range.endMs)return false;return true}
function filterTimeSeriesPoints(xs,ys,range){const outX=[];const outY=[];for(let i=0;i<(xs||[]).length;i+=1){if(timeInDashboardRange(xs[i],range)){outX.push(xs[i]);outY.push((ys||[])[i])}}return{x:outX,y:outY}}
function filterTradeLinksByRange(xs,ys,range){const outX=[];const outY=[];for(let i=0;i<(xs||[]).length;i+=3){const x0=xs[i];const x1=xs[i+1];const y0=(ys||[])[i];const y1=(ys||[])[i+1];if(x0==null||x1==null)continue;if(!timeInDashboardRange(x0,range)||!timeInDashboardRange(x1,range))continue;outX.push(x0,x1,null);outY.push(y0,y1,null)}return{x:outX,y:outY}}
function filterPriceByRange(price){const range=currentTimeFilterRange();if(!price||!price.x?.length)return price;if(!Number.isFinite(range.startMs)&&!Number.isFinite(range.endMs))return price;let startIndex=-1;let endIndex=-1;for(let i=0;i<price.x.length;i+=1){if(!timeInDashboardRange(price.x[i],range))continue;if(startIndex<0)startIndex=i;endIndex=i}if(startIndex<0){return{...price,x:[],open:[],high:[],low:[],close:[],capital_x:[],capital_y:[]}}const capitalPoints=filterTimeSeriesPoints(price.capital_x||[],price.capital_y||[],range);return{...price,x:price.x.slice(startIndex,endIndex+1),open:price.open.slice(startIndex,endIndex+1),high:price.high.slice(startIndex,endIndex+1),low:price.low.slice(startIndex,endIndex+1),close:price.close.slice(startIndex,endIndex+1),capital_x:capitalPoints.x,capital_y:capitalPoints.y}}
function filterTransByRange(trans){const range=currentTimeFilterRange();if(!trans)return trans;if(!Number.isFinite(range.startMs)&&!Number.isFinite(range.endMs))return trans;const buyPoints=filterTimeSeriesPoints(trans.buy_points?.x||[],trans.buy_points?.y||[],range);const sellWdPoints=filterTimeSeriesPoints(trans.sell_wd_points?.x||[],trans.sell_wd_points?.y||[],range);const sellSpeedPoints=filterTimeSeriesPoints(trans.sell_speed_points?.x||[],trans.sell_speed_points?.y||[],range);const capitalPoints=filterTimeSeriesPoints(trans.capital_x||[],trans.capital_y||[],range);const tradeLinks=filterTradeLinksByRange(trans.trade_link_x||[],trans.trade_link_y||[],range);return{...trans,buy_points:buyPoints,sell_wd_points:sellWdPoints,sell_speed_points:sellSpeedPoints,capital_x:capitalPoints.x,capital_y:capitalPoints.y,trade_link_x:tradeLinks.x,trade_link_y:tradeLinks.y}}
function ensureTimeFilterControl(){const grid=document.querySelector("#pageDetail .controls-grid");if(!grid)return;let host=$("timeFilterHost");if(!host){host=document.createElement("div");host.id="timeFilterHost";host.className="time-range-host";host.innerHTML='<div class="label">展示时间</div><div class="time-range-row"><select id="inputTimeStart" class="sel"><option value="">全部</option></select><div class="time-range-dash">-</div><select id="inputTimeEnd" class="sel"><option value="">全部</option></select></div>';const wdHost=$("inputWdHost")||$("inputWd")?.parentElement;if(wdHost&&wdHost.nextSibling){grid.insertBefore(host,wdHost.nextSibling)}else{grid.appendChild(host)}}const startNode=$("inputTimeStart");const endNode=$("inputTimeEnd");if(startNode&&!startNode.dataset.bound){startNode.dataset.bound="1";startNode.addEventListener("change",()=>applyTimeFilter("start"))}if(endNode&&!endNode.dataset.bound){endNode.dataset.bound="1";endNode.addEventListener("change",()=>applyTimeFilter("end"))}}
function applyTimeFilter(source){const startNode=$("inputTimeStart");const endNode=$("inputTimeEnd");if(!startNode||!endNode)return;let start=String(startNode.value||"");let end=String(endNode.value||"");if(start&&!state.timeFilterStartOptions.includes(start)){start=""}if(end&&!state.timeFilterEndOptions.includes(end)){end=""}if(start&&end&&parseDashboardTime(start)>parseDashboardTime(end)){if(source==="start"){end="";endNode.value=""}else{start="";startNode.value=""}}state.timeFilterStart=start;state.timeFilterEnd=end;if(state.activeRecord){setRecord(state.activeRecord,false)}}
const setupLayoutTimeFilter=setupLayout;setupLayout=function(){setupLayoutTimeFilter();ensureTimeFilterControl()}
const ensurePriceTimeFilter=ensurePrice;ensurePrice=async function(record){const data=await ensurePriceTimeFilter(record);syncTimeFilterControls(data?.x||[]);return data}
const priceChartTimeFilter=priceChart;priceChart=function(price,trans){const filteredPrice=filterPriceByRange(price);const filteredTrans=filterTransByRange(trans);if(price&&Array.isArray(price.x)&&price.x.length&&filteredPrice&&Array.isArray(filteredPrice.x)&&!filteredPrice.x.length){$("priceChart").style.display="none";$("priceEmpty").style.display="grid";$("priceEmpty").innerHTML="当前展示时间无数据";return}priceChartTimeFilter(filteredPrice,filteredTrans)}
const resetUiTimeFilter=resetUi;resetUi=function(){resetUiTimeFilter();state.timeFilterStart="";state.timeFilterEnd="";state.timeFilterStartOptions=[];state.timeFilterEndOptions=[];const startNode=$("inputTimeStart");const endNode=$("inputTimeEnd");if(startNode){fillTimeFilterSelect(startNode,[]);startNode.value=""}if(endNode){fillTimeFilterSelect(endNode,[]);endNode.value=""}}
setupLayout();
bootstrapPreset();
</script>'''
)
...
PAGE = PAGE.replace(
    "</style>",
    """
.controls-grid{grid-template-columns:minmax(110px,.78fr) minmax(110px,.92fr) minmax(110px,.92fr) minmax(110px,.92fr) minmax(300px,1.7fr)}
.time-range-host{width:100%;align-self:stretch}
.time-range-row{width:100%}
.price-actions-left{display:flex;flex-direction:column;align-items:flex-start;gap:12px}
.price-actions-right{align-items:flex-end}
.compare-panel{display:grid;gap:12px;width:100%;max-width:none;margin-bottom:16px}
.compare-panel-grid{display:grid;gap:12px;grid-template-columns:repeat(4,minmax(0,1fr))}
.compare-panel-grid .sel{width:100%;box-sizing:border-box;min-width:0}
.compare-box .chart{min-height:520px}
@media(max-width:1400px){
  .controls-grid{grid-template-columns:repeat(3,minmax(0,1fr))}
  .compare-panel-grid{grid-template-columns:repeat(2,minmax(0,1fr))}
}
</style>"""
)
...
PAGE = PAGE.replace(
    'bootstrapPreset();\n</script>',
    '''state.compareRecord=state.compareRecord||null;
function compareFieldDefs(){return[{id:"compareInputBar",field:"open_bar"},{id:"compareInputThreshold",field:"open_threshold"},{id:"compareInputCont",field:"open_continous_threshold"},{id:"compareInputWd",field:"withdrawal_limit"}]}
function compareSelections(){const out={};for(const def of compareFieldDefs()){out[def.field]=keyOf($(def.id)?.value)}return out}
function hasAnyCompareSelection(){return compareFieldDefs().some(def=>!!keyOf($(def.id)?.value))}
function compareLabelText(field){const labels=(typeof currentProgramLabelMapFinal==="function"?currentProgramLabelMapFinal():typeof currentParamLabelMapFinal==="function"?currentParamLabelMapFinal():null)||{};return labels[field]||field}
function ensureCompareGroupUi(){const detailShell=document.querySelector("#pageDetail .detail-shell");const priceBox=document.querySelector("#pageDetail .price-box");let box=$("compareChartBox");if(!box&&detailShell&&priceBox){box=document.createElement("div");box.id="compareChartBox";box.className="chartbox compare-box";box.innerHTML='<div class="charttitle" id="compareChartTitle">对比组</div><div id="comparePriceEmpty" class="empty">请选择对比组参数</div><div id="comparePriceChart" class="chart" style="display:none"></div>';if(priceBox.nextSibling){detailShell.insertBefore(box,priceBox.nextSibling)}else{detailShell.appendChild(box)}}let panel=$("comparePanel");if(!panel){panel=document.createElement("div");panel.id="comparePanel";panel.className="compare-panel";panel.innerHTML='<div class="compare-panel-grid"><div id="compareInputBarHost"><div class="label">时间窗口</div><select id="compareInputBar" class="sel"></select></div><div id="compareInputThresholdHost"><div class="label">速度限制</div><select id="compareInputThreshold" class="sel"></select></div><div id="compareInputContHost"><div class="label">开仓门槛</div><select id="compareInputCont" class="sel"></select></div><div id="compareInputWdHost"><div class="label">回撤限制</div><select id="compareInputWd" class="sel"></select></div></div>'}if(box&&panel.parentElement!==box){box.insertBefore(panel,$("comparePriceEmpty"))}for(const def of compareFieldDefs()){const node=$(def.id);if(node&&!node.dataset.bound){node.dataset.bound="1";node.addEventListener("change",()=>{renderCompareGroup()})}}syncCompareLabels()}
function syncCompareLabels(){const hostMap={compareInputBarHost:"open_bar",compareInputThresholdHost:"open_threshold",compareInputContHost:"open_continous_threshold",compareInputWdHost:"withdrawal_limit"};for(const hostId of Object.keys(hostMap)){const labelNode=document.getElementById(hostId)?.querySelector(".label");if(labelNode){labelNode.textContent=compareLabelText(hostMap[hostId])}}}
function fillCompareSelect(node,values,selectedValue){if(!node)return;node.innerHTML="";const first=document.createElement("option");first.value="";first.textContent="请选择";node.appendChild(first);for(const value of values){const option=document.createElement("option");option.value=keyOf(value);option.textContent=formatParamDisplay(value);node.appendChild(option)}node.disabled=!values.length;node.value=values.map(keyOf).includes(keyOf(selectedValue))?keyOf(selectedValue):""}
function syncCompareControlOptions(){ensureCompareGroupUi();if(!state.summary?.records?.length){for(const def of compareFieldDefs()){fillCompareSelect($(def.id),[],"")}return}const current=compareSelections();for(const def of compareFieldDefs()){const values=state.summary?.controls?.[def.field]?.values||[];fillCompareSelect($(def.id),values,current[def.field])}}
function compareRecordFromSelection(){if(!state.summary?.records?.length){return null}const selections=compareSelections();if(compareFieldDefs().some(def=>!selections[def.field])){return null}return state.summary.records.find(row=>compareFieldDefs().every(def=>keyOf(row?.[def.field])===selections[def.field]))||null}
function clearCompareGroup(message){const chart=$("comparePriceChart");const empty=$("comparePriceEmpty");const title=$("compareChartTitle");if(chart){chart.style.display="none"}if(empty){empty.style.display="grid";empty.innerHTML=message||"请选择对比组参数"}if(title){title.textContent="对比组"}state.compareRecord=null}
function buildFilteredPriceChartData(price,trans){const filteredPrice=filterPriceByRange(price);const filteredTrans=filterTransByRange(trans);return{price:filteredPrice,trans:filteredTrans}}
function plotComparePriceChart(price,trans){const chart=$("comparePriceChart");const empty=$("comparePriceEmpty");if(!chart||!empty)return;const filtered=buildFilteredPriceChartData(price,trans);const dataPrice=filtered.price;const dataTrans=filtered.trans;if(price&&Array.isArray(price.x)&&price.x.length&&dataPrice&&Array.isArray(dataPrice.x)&&!dataPrice.x.length){clearCompareGroup("当前展示时间无数据");return}if(!dataPrice||!dataPrice.x?.length){clearCompareGroup("无数据");return}empty.style.display="none";chart.style.display="block";const candleX=dataPrice.x.map((_,index)=>index);const priceIndexMap=buildIndexMap(dataPrice.x);const shapes=gapShapes(dataPrice.x);const perfCapitalPoints=axisPoints(dataPrice.capital_x||[],dataPrice.capital_y||[],priceIndexMap);const basePrice=Number(dataPrice.price_base);const scaleValue=value=>{if(value===null||value===undefined||value==="")return value;const num=Number(value);if(!Number.isFinite(num)||!Number.isFinite(basePrice)||basePrice===0)return num;return num/basePrice*100};const scaleAxisPoints=points=>({x:[...(points?.x||[])],y:(points?.y||[]).map(scaleValue),text:[...(points?.text||[])]});const traces=[{type:"candlestick",x:candleX,open:dataPrice.open,high:dataPrice.high,low:dataPrice.low,close:dataPrice.close,text:dataPrice.x,name:"price",increasing:{line:{color:CANDLE_UP_EDGE,width:0.8},fillcolor:CANDLE_UP_FILL},decreasing:{line:{color:CANDLE_DOWN_EDGE,width:0.8},fillcolor:CANDLE_DOWN_FILL},hovertemplate:"%{text}<br>open=%{open:.4f}<br>high=%{high:.4f}<br>low=%{low:.4f}<br>close=%{close:.4f}<extra></extra>"}];const capitalPoints=perfCapitalPoints.x.length?perfCapitalPoints:(dataTrans?axisPoints(dataTrans.capital_x,dataTrans.capital_y,priceIndexMap):{x:[],y:[],text:[]});if(state.showCapitalOverlay&&capitalPoints.x.length)traces.unshift({type:"scatter",mode:"lines",x:capitalPoints.x,y:capitalPoints.y,text:capitalPoints.text,line:{color:ACCENT_BLUE,width:1.2},name:"capital",hovertemplate:"%{text}<br>capital=%{y:.4f}<extra></extra>"});if(dataTrans){const tradeLink=scaleAxisPoints(axisPoints(dataTrans.trade_link_x,dataTrans.trade_link_y,priceIndexMap,true));const buyPoints=scaleAxisPoints(axisPoints(dataTrans.buy_points.x,dataTrans.buy_points.y,priceIndexMap));const sellWdPoints=scaleAxisPoints(axisPoints(dataTrans.sell_wd_points.x,dataTrans.sell_wd_points.y,priceIndexMap));const sellSpeedPoints=scaleAxisPoints(axisPoints(dataTrans.sell_speed_points.x,dataTrans.sell_speed_points.y,priceIndexMap));if(tradeLink.x.length)traces.push({type:"scatter",mode:"lines",x:tradeLink.x,y:tradeLink.y,line:{color:ACCENT_BLUE,width:2},hoverinfo:"skip",name:"trade_link"});if(buyPoints.x.length)traces.push({type:"scatter",mode:"markers",x:buyPoints.x,y:buyPoints.y,text:buyPoints.text,marker:{color:"red",size:4},name:"buy",hovertemplate:"buy<br>%{text}<br>base100=%{y:.4f}<extra></extra>"});if(sellWdPoints.x.length)traces.push({type:"scatter",mode:"markers",x:sellWdPoints.x,y:sellWdPoints.y,text:sellWdPoints.text,marker:{color:SELL_WD_COLOR,size:4},name:"sell_wd",hovertemplate:"sell_wd<br>%{text}<br>base100=%{y:.4f}<extra></extra>"});if(sellSpeedPoints.x.length)traces.push({type:"scatter",mode:"markers",x:sellSpeedPoints.x,y:sellSpeedPoints.y,text:sellSpeedPoints.text,marker:{color:SELL_SPEED_COLOR,size:4},name:"sell_speed",hovertemplate:"sell_speed<br>%{text}<br>base100=%{y:.4f}<extra></extra>"})}Plotly.newPlot(chart,traces,{margin:{l:48,r:18,t:12,b:36},paper_bgcolor:"rgba(0,0,0,0)",plot_bgcolor:"#fff",xaxis:{...numericAxis(dataPrice.x),showticklabels:false,ticks:"",title:""},yaxis:{title:"price (base=100)",gridcolor:"#e8eef8",zeroline:false},legend:{orientation:"h",yanchor:"bottom",y:1.02,xanchor:"left",x:0},shapes:shapes},{responsive:true,displayModeBar:false})}
async function renderCompareGroup(){ensureCompareGroupUi();syncCompareLabels();const title=$("compareChartTitle");const record=compareRecordFromSelection();if(!hasAnyCompareSelection()){clearCompareGroup("请选择对比组参数");return}if(!record){clearCompareGroup("无数据");return}state.compareRecord=record;if(title){title.textContent="对比组  "+formatParamTag(record.param_tag||"")}try{const [price,trans]=await Promise.all([ensurePrice(record),ensureTrans(record)]);plotComparePriceChart(price,trans)}catch(error){clearCompareGroup("详情载入失败：<br>"+esc(error.message))}}
const setupLayoutCompareGroup=setupLayout;setupLayout=function(){setupLayoutCompareGroup();ensureCompareGroupUi();syncCompareControlOptions()}
const syncProgramSpecificLabelsCompareGroup=syncProgramSpecificLabels;syncProgramSpecificLabels=function(){syncProgramSpecificLabelsCompareGroup();syncCompareLabels()}
const loadSummaryCompareGroup=loadSummary;loadSummary=async function(file){const result=await loadSummaryCompareGroup(file);syncCompareControlOptions();clearCompareGroup("请选择对比组参数");return result}
const resetUiCompareGroup=resetUi;resetUi=function(){resetUiCompareGroup();for(const def of compareFieldDefs()){const node=$(def.id);if(node){node.innerHTML='<option value="">请选择</option>';node.value=""}}clearCompareGroup("请选择对比组参数")}
const setRecordCompareGroup=setRecord;setRecord=async function(record,switchTab){await setRecordCompareGroup(record,switchTab);syncCompareControlOptions();if(hasAnyCompareSelection()){await renderCompareGroup()}}
setupLayout();
bootstrapPreset();
</script>'''
)
...
PAGE = PAGE.replace(
    "</style>",
    """
.controls-card,.price-box,.compare-box{width:100%;max-width:none;box-sizing:border-box}
.detail-shell>.chartbox{width:100%;box-sizing:border-box}
.controls-grid{grid-template-columns:repeat(4,minmax(0,1fr)) minmax(0,1.45fr);align-items:start}
.drawer.collapsed .drawer-head{align-items:flex-start}
.drawer.collapsed .drawer-toggle{margin-top:-24px}
@media(max-width:1400px){
  .controls-grid{grid-template-columns:repeat(3,minmax(0,1fr))}
}
</style>"""
...
PAGE = PAGE.replace(
    'bootstrapPreset();\n</script>',
    '''function formatPercentMetric2(value){const text=formatMetric2(value);return text==="-"?text:text+"%"}
const syncPriceHeaderStatsPercent=syncPriceHeaderStats;
syncPriceHeaderStats=function(record){syncPriceHeaderStatsPercent(record);const wdNode=$("priceHeadWd");const avgNode=$("priceHeadAvgProfit");const finalNode=$("priceHeadFinalProfit");if(wdNode){wdNode.textContent=formatPercentMetric2(record?.biggest_wd)}if(avgNode){avgNode.textContent=formatPercentMetric2(avgProfitPerTrade(record))}if(finalNode){finalNode.textContent=formatPercentMetric2(capitalProfit(record))}}
const applyTimeFilterCompareLink=applyTimeFilter;
applyTimeFilter=function(source){applyTimeFilterCompareLink(source);if(!state.activeRecord&&typeof hasAnyCompareSelection==="function"&&hasAnyCompareSelection()){renderCompareGroup()}}
bootstrapPreset();
</script>'''
)
